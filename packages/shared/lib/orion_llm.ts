...
import { LLMRequestOptions } from '@shared/types/llm';
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (shouldRetry(error)) {
        ...
        console.warn(`[LLM_RETRY] Attempt ${i + 1}/${retries} failed with retryable error. Retrying in ${Math.round(delay + jitter)}ms...`, lastError.message);
...
export async function generateLLMResponse(
  ...
  try {
    const result = await callLLMWithFallback(messages, primaryModel, temperature, maxTokens);
    if (!result || !result.choices || result.choices.length === 0) {
      throw new Error('LLM response was empty or invalid.');
    }
    // Handle both Azure and other provider response structures
    const content = result.choices[0]?.message?.content || result.choices[0]?.text || null;
    if (content === null) {
    ...
    throw new Error(`Failed to generate LLM response after trying all fallbacks: ${error.message}`);
  }
}
