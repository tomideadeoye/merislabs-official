...
import { OrionOpportunityNotionOutputShared } from '@repo/shared';
...
console.log(
  `[NOTION_SERVICE][DEBUG] Imported NOTION_API_KEY: ${
    NOTION_API_KEY ? "Loaded" : "[NOT SET]"
  }`
);
console.log(
  `[NOTION_SERVICE][DEBUG] Imported NOTION_DATABASE_ID: ${
...
function getTitleFromProperties(properties: any): string {
  ...
  return (
    properties["Aa Title"]?.title?.[0]?.plain_text ||
    ...
    const response = await notion.pages.create({
      ...
      properties: {
        ...
        companyOrInstitution: {
          rich_text: [{ text: { content: data.companyOrInstitution } }],
        },
        URL: { url: data.url },
...
export async function listOpportunitiesFromNotion(filters: any = {}) {
  ...
  if (!notion || !NOTION_DATABASE_ID) {
    console.error(
      "[NOTION][listOpportunitiesFromNotion][ERROR] Notion client or Database ID not configured."
    );
    return [];
  }
  ...
  try {
    ...
    console.log(
      `[NOTION][listOpportunitiesFromNotion][SUCCESS] Found ${response.results.length} opportunities.`
    );

    // Map raw Notion pages to a structured OrionOpportunity object
    return response.results.map((page: any) => {
      const properties = page.properties;
      return {
        id: page.id,
        title:
          getTitleFromProperties(properties) ||
          properties.companyOrInstitution?.rich_text?.[0]?.text?.content ||
          "Unknown Title",
        company:
          properties.companyOrInstitution?.rich_text?.[0]?.text?.content || "",
        companyOrInstitution:
...
export async function getOpportunityContent(pageId: string) {
  ...
  try {
    ...
    console.error(
      `Failed to retrieve content for OrionOpportunity ${pageId}:`,
      error
    );
    return null;
  }
          ...
          rich_text: [
            {
              text: {
                ...
                link: stakeholder.linkedin_url
                  ? { url: stakeholder.linkedin_url }
                  : undefined,
              },
            },
          ],
...
export async function getCVComponentsFromNotion(): Promise<
  CVComponentShared[]
> {
  if (!notion || !NOTION_DATABASE_ID) {
    console.warn(
  ...
  try {
    ...
    return response.results.map((page: any) => {
      ...
      return {
        ...
        content_primary:
          properties["Content (Primary)"]?.rich_text?.[0]?.text?.content,
        id: page.id,
    ...
    console.error(
      "[NOTION][getCVComponentsFromNotion][ERROR] Failed to get CV components from Notion",
      error
    );
    return [];
  }
...
export async function fetchOpportunityByIdFromNotion(
  ...
  logger.info(
    "[NOTION][fetchOpportunityByIdFromNotion][START] Fetching OrionOpportunity",
    { opportunityId }
  );
  if (!notion) {
    logger.error(
      "[NOTION][fetchOpportunityByIdFromNotion][ERROR] Notion client not configured",
      { opportunityId }
    );
  ...
  if (!opportunityId || typeof opportunityId !== "string") {
    logger.error(
      "[NOTION][fetchOpportunityByIdFromNotion][ERROR] Invalid opportunityId",
      { opportunityId }
    );
    return { success: false, error: "Invalid OrionOpportunity ID" };
  }
  try {
    ...
    const OrionOpportunity = {
      ...
      title:
        getTitleFromProperties(properties) ||
        (properties.companyOrInstitution as any)?.rich_text?.[0]?.text
          ?.content ||
        "Unknown Title",
      company:
        (properties.companyOrInstitution as any)?.rich_text?.[0]?.text
      ...
      last_edited_time: page.last_edited_time,
      content: (properties.Content as any)?.rich_text
    ...
    logger.success(
      "[NOTION][fetchOpportunityByIdFromNotion][SUCCESS] OrionOpportunity fetched",
      { opportunityId }
    );
    return {
      success: true,
      OrionOpportunity: OrionOpportunity as OrionOpportunityNotionOutputShared,
    };
  } catch (error: any) {
    ...
    if (
      ...
      errorMsg = "OrionOpportunity not found in Notion";
    } else if (error instanceof Error) {
      errorMsg = error.message;
    }
    logger.error(
      "[NOTION][fetchOpportunityByIdFromNotion][ERROR] Failed to fetch OrionOpportunity",
      { opportunityId, error: errorMsg, raw: error }
    );
...
export async function updateNotionOpportunity(
  pageId: string,
  properties: any
): Promise<{ success: boolean; OrionOpportunity?: any; error?: string }> {
  if (!notion) {
    console.warn(
  ...
  try {
    ...
    console.error(
      `[NOTION][updateNotionOpportunity][ERROR] Failed to update OrionOpportunity ${pageId}`,
      error
    );
    return {
      success: false,
...
export async function getJournalEntriesFromNotion() {
  if (!notion || !NOTION_DATABASE_ID) {
    console.error(
      "[NOTION][getJournalEntriesFromNotion][ERROR] Notion client or Database ID not configured."
    );
    return [];
  }
  ...
  try {
    ...
    return response.results.map((page: any) => {
      ...
      return {
        ...
        contentType:
          properties["Content Type"]?.select?.name || "journal_entry",
      };
    });
  } catch (error) {
...
export async function fetchContactsFromNotion() {
  console.warn(
    "[NOTION][fetchContactsFromNotion] This function is not yet implemented."
  );
  return [];
}
...
export async function saveJournalEntryToNotion(data: any) {
  console.warn(
    "[NOTION][saveJournalEntryToNotion] This function is deprecated. Use createJournalEntryInNotion instead."
  );
...
export async function updateNotionDatabaseSchema(
  ...
  console.warn(
    "[NOTION][updateNotionDatabaseSchema] This function is not yet implemented."
  );
