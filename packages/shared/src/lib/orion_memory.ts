...
import type { ScoredMemoryPoint, QdrantFilter } from '@repo/shared';
 ...
 */
export async function processTextForIndexing(
  text: string,
  sourceId: string,
  tags: string[] = []
) {
...
export async function searchMemory(params: {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      console.error(
        "Error in memory search API response:",
        response.data.error
      );
      throw new Error(
        response.data.error || "Memory search API returned an error."
      );
    }
  } catch (error: any) {
