  ...
  const filteredOpportunities = useMemo(() => {
    return opportunities
      .filter(opp => {
        ...
        const matchesTag = !filters.tag || (opp.tags && opp.tags.some((tag: string) => tag.toLowerCase().includes(filters.tag.toLowerCase())));
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredOpportunities.map((OrionOpportunity) => (
            <OpportunityCard key={OrionOpportunity.id} OrionOpportunity={OrionOpportunity} />
          ))}
        </div>
      )}
