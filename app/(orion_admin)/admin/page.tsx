...
interface LlmApiKeyStatus extends LLMModelConfig {
  present: boolean;
  provider: string;
}
...
export default function AdminDashboardPage() {
  ...
  const [profileErrorState, setProfileErrorState] = useState<string | null>(profileError?.message || null);
  ...
  const [llmHealth, setLlmHealth] = useState<LlmHealthStatus[]>([]);
  ...
  useEffect(() => {
    async function fetchHealth() {
      ...
      try {
        ...
        setLlmHealthError(err instanceof Error ? err.message : 'An unknown error occurred');
      } finally {
  ...
  useEffect(() => {
    ...
    setProfileErrorState(profileError?.message || null);
  ...
  const handleRefreshProfile = async () => {
    ...
    try {
      ...
      consolidatedLogger.info('[ProfileUI][ACTION] Removed profile cache, fetching fresh from API...');
      ...
      consolidatedLogger.info('[ProfileUI][ACTION] Profile refreshed and cached', { data });
    } catch (err: unknown) {
      consolidatedLogger.error(
        '[ProfileUI][ERROR] Failed to refresh profile:',
        err instanceof Error ? err : new Error(String(err))
      );
      setProfileErrorState(err instanceof Error ? err.message : 'An unknown error occurred');
      setProfileData(null);
    } finally {
    ...
    <div className="space-y-8">
          ...
          <CardHeader
            ...
            onClick={() => {
              setProfileExpanded((prev) => {
                const next = !prev;
                consolidatedLogger.info('[ProfileUI][ACTION] Toggled profile card expand/collapse', { expanded: next });
                return next;
              });
            }}
          >
            <div className="flex items-center text-xl text-white">
              ...
              <span className="ml-2">
                {profileExpanded ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
              </span>
            </div>
            <button
              type="button"
              ...
              {!profileLoadingState && !profileErrorState && profileData && (
                <div className="space-y-2 text-white">
                  ...
                  <div className="mt-2 text-xs text-gray-400">
                    (Full object: <code>{JSON.stringify(profileData)}</code>)
                  </div>
                </div>
              )}
              ...
              {!profileLoadingState && (profileErrorState || !profileData) && (
                <div className="flex flex-col items-center justify-center text-center py-6">
                  ...
                  <div className="text-gray-400 mb-4">
                    Orion can&apos;t find your profile. This may be due to a network issue, missing Notion config, or
                    local file fallback failure.
                    <br />
              ...
              <div className="mt-4 flex gap-2">
                ...
                <Button
                  ...
                  onClick={(e) => {
                    ...
                    setProfileErrorState(null);
                    setProfileLoadingState(false);
                    consolidatedLogger.info('[ProfileUI][ACTION] Cleared profile cache (no reload).');
                  }}
                  disabled={refreshing}
                >
                  Clear Profile Cache
        ...
        <CardContent>
          ...
          <div className="mb-4 flex flex-wrap gap-4 items-center">
            <span className="text-green-400 font-bold flex items-center">
              <span className="mr-1">✅</span>OK: {llmHealth.filter((r) => r.status === 'success').length}
            </span>
            <span className="text-red-400 font-bold flex items-center">
              <span className="mr-1">❌</span>Fail: {llmHealth.filter((r) => r.status === 'fail').length}
            </span>
            <span className="text-yellow-400 font-bold flex items-center">
              <span className="mr-1">⚠️</span>Missing Key: {llmApiKeys.filter((k) => !k.present).length}
            </span>
            <span className="text-gray-400 ml-4">
              Total Models: {Object.values(PROVIDER_MODEL_CONFIGS).flat().length}
            </span>
          </div>
                    ...
                    models.map((model: LLMModelConfig) => {
                      ...
                      const healthBadge = (() => {
                        if (!keyStatus?.present) {
                          ...
                          return (
                            <span className="bg-red-900 text-red-300 px-2 py-1 rounded text-xs font-mono">FAIL</span>
                          );
                        } else {
                          return (
                      ...
                      return (
                        <tr key={model.modelId} className="border-b border-gray-800 hover:bg-gray-900/40 transition">
                          ...
                          <td className="py-2 pr-4">
                            {apiKeyBadge} <span className="ml-2 text-xs text-gray-500">{model.apiKeyEnv}</span>
                          </td>
                          <td className="py-2 pr-4">{healthBadge}</td>
                          <td className="py-2 text-xs text-gray-400 max-w-xs truncate">
                            {healthStatus?.status === 'fail' && (
                              <span title={healthStatus.error || ''}>
                                {healthStatus.error && healthStatus.error.length > 60
                                  ? healthStatus.error.slice(0, 60) + '...'
                                  : healthStatus.error}
                              </span>
                            )}
          ...
          {llmApiKeys.some((k) => !k.present) && (
            <div className="mt-4 text-yellow-400 font-bold flex items-center">
              <span className="mr-2">⚠️</span>One or more LLM API keys are missing. Add them to your .env.local and
              restart the server.
            </div>
          )}
        </CardContent>
      ...
      <section className="container mx-auto pb-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Habitica Integration</CardTitle>
              <CardDescription>View and manage your Habitica tasks and stats</CardDescription>
            </CardHeader>
            <CardContent>
              <HabiticaStatsContainer />
              <p className="mb-4 mt-4">Integrate with Habitica to track your habits, dailies, and todos.</p>
              <Link href="/admin/habitica">
                <Button>Open Habitica Dashboard</Button>
              </Link>
            </CardContent>
          </Card>
        </div>
      ...
      <div className="mt-8">
        ...
        <p className="text-gray-400 mb-4">Browse and interact with your local file system.</p>
        <Button asChild>
          <Link href="/admin/local-files">Open File Explorer</Link>
        </Button>
      </div>
    </div>
  );
