...
export async function PATCH(request: Request, { params }: { params: { opportunityId: string } }) {
  ...
  if (updatedOpportunity) {
    ...
    console.log('[PATCH /api/orion/notion/OrionOpportunity/[opportunityId]] Sending error response:', errorResponse);
...
export async function GET(request: Request, { params }: { params: { opportunityId: string } }) {
  ...
  if (!opportunityId) {
    console.error(`[GET /api/orion/notion/OrionOpportunity/${opportunityId}] OrionOpportunity ID is missing.`);
    return NextResponse.json({ success: false, error: 'OrionOpportunity ID is required.' }, { status: 400 });
  }

  try {
    console.log(`[GET /api/orion/notion/OrionOpportunity/${opportunityId}] Calling fetchOpportunityByIdFromNotion...`);
    ...
    if (fetchResult.success) {
      ...
      console.info(`[GET /api/orion/notion/OrionOpportunity/${opportunityId}] Normalized company fields:`, {
        company,
        companyOrInstitution,
      });
      console.log(
