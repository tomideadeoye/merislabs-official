 ...
 */
async function findPotentialStakeholders(query: string, roles?: string[]): Promise<Stakeholder[]> {
  // Get company info
  ...
  const stakeholdersPromises = searchRoles.map((role) => searchLinkedInProfiles(query, role));
