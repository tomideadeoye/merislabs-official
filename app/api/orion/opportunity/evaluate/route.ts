  ...
  try {
    ...
    if (!opportunityDetails.title || !opportunityDetails.content || !opportunityDetails.type) {
      return NextResponse.json(
        {
          success: false,
          error: 'Missing required OrionOpportunity details: title, description, and type.',
        },
        { status: 400 }
      );
    }
...
`;
    ...
    if (!llmData.success || !llmData.content) {
      throw new Error(llmData.error || 'Failed to generate OrionOpportunity evaluation');
    }
    ...
    try {
      ...
      if (jsonString.includes('```json')) {
        jsonString = jsonString.substring(jsonString.indexOf('```json') + 7, jsonString.lastIndexOf('```')).trim();
      } else if (jsonString.includes('```')) {
        jsonString = jsonString.substring(jsonString.indexOf('```') + 3, jsonString.lastIndexOf('```')).trim();
      }
