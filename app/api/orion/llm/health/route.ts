...
export async function GET() {
  ...
  for (const model of MODELS_TO_CHECK) {
    try {
      const response = await callExternalLLM(model, [{ role: 'user', content: 'Say hello.' }], 0.2, 32);
      results.push({
        model,
  ...
  const [deepseekHealthy, ollamaHealthy] = await Promise.all([checkDeepSeekHealth(), checkOllamaHealth()]);
...
async function checkDeepSeekHealth() {
  ...
  try {
    ...
    const response = await fetch(`${config.basePath}?api-version=${config.apiVersion}`, {
      method: 'POST',
      headers: {
