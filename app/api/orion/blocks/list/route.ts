  ...
  try {
    ...
    if (tagsParam) {
      try {
        ...
        console.error('[BLOCKS_LIST_API] Malformed tags parameter:', error);
    ...
    if (!searchResult.success) {
      return NextResponse.json(
        {
          success: false,
          error: searchResult.error || 'Failed to list blocks',
        },
        { status: 500 }
      );
    }
    ...
    const isBlockType = (type: string): type is BlockType => (BLOCK_TYPES as readonly string[]).includes(type);
    ...
    if (
      ...
      (error instanceof Error && error.message?.toLowerCase().includes('malformed'))
    ) {
      return NextResponse.json(
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Failed to list blocks' },
      { status: 500 }
    );
  }
}
