...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!session || !session.user || !session.user.id) {
      logger.warn('[CV_ASSEMBLE_API][POST][AUTH_FAIL]', logContext);
      return NextResponse.json({ success: false, error: 'Unauthorized' }, { status: 401 });
    }
    const userId = session.user.id; // Get userId
    ...
    if (autoGenerate) {
      ...
      const allCvComponents = await prisma.cVComponent.findMany({
        where: { userId: userId }, // Ensure we only fetch components for the current user
      });
      ...
      assembledComponents = autoGeneratedOutput.selectedComponents.map((selectedComp) => {
        ...
        const originalComp = allCvComponents.find((c) => c.uniqueId === selectedComp.id);
        const contentToUse =
      ...
      const dbComponents = await prisma.cVComponent.findMany({
        where: { uniqueId: { in: uniqueIdsToFetch } },
      });
      ...
      const dbComponentsMap = new Map(dbComponents.map((c) => [c.uniqueId, c]));
