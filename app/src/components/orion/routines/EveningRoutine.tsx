...
import { DAILY_REFLECTION_REQUEST_TYPE } from '@repo/shared/lib/orion_config';
...
export const EveningRoutine: React.FC = () => {
  ...
  const [completedTasks, setCompletedTasks] = useState<any[]>([]); // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const fetchCompletedTasks = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const todaysTasks = data.tasks.filter((task: { dateCompleted: string | null }) => {
    ...
    <div className="space-y-6">
      {eveningRoutineCompleted ? (
        <div className="bg-green-900/30 border border-green-700 text-green-300 p-3 rounded-md flex items-center">
          ...
          Your Evening Routine is complete! Well done.
        </div>
      ) : (
        <>
