...
import { Progress } from '@/ui/components/ui';
...
export const RoutineStatus: React.FC<RoutineStatusProps> = ({ className }) => {
  ...
  const lastRun = getSessionValue(SessionStateKeys.ROUTINES_LAST_RUN) || 'N/A';
  const morningCompleted = getSessionValue(SessionStateKeys.ROUTINES_MORNING_COMPLETED) || false;
  const eveningCompleted = getSessionValue(SessionStateKeys.ROUTINES_EVENING_COMPLETED) || false;
  const executionStatus = getSessionValue(SessionStateKeys.ROUTINES_EXECUTION_STATUS) || 'Idle';
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center">
            ...
            <h4
              className={`font-medium ${
                morningCompleted ? 'text-green-300' : highlightMorning ? 'text-blue-300' : 'text-gray-400'
              }`}
            >
              Morning Kickstart
            </h4>
          ...
          <p className="text-xs mt-1 text-gray-400">
            {morningCompleted ? 'Completed' : 'Log mood, review tasks, set intentions'}
          </p>
        </div>
        ...
        >
          <div className="flex items-center">
            ...
            <h4
              className={`font-medium ${
                eveningCompleted ? 'text-green-300' : highlightEvening ? 'text-purple-300' : 'text-gray-400'
              }`}
            >
              Evening Wind-Down
            </h4>
          ...
          <p className="text-xs mt-1 text-gray-400">
            {eveningCompleted ? 'Completed' : 'Review accomplishments, reflect, log mood'}
          </p>
        </div>
      </div>
    </div>
