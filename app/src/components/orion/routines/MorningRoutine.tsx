...
import { Button, Checkbox } from '@/ui/components/ui';
import { Loader2, CheckCircle, Brain, ListChecks, Sparkles } from 'lucide-react';
import { THOUGHT_FOR_THE_DAY_REQUEST_TYPE } from '@/lib/orion_config';
import Link from 'next/link';
import { SessionStateKeys } from '@/lib/app_constants';
import { startMorningRoutine, stopMorningRoutine } from '@/lib/routines';
...
export const MorningRoutine: React.FC = () => {
  // Session state
  const { getSessionValue, setSessionValue } = useSessionState();
  ...
  const morningRoutineCompleted = getSessionValue(SessionStateKeys.ROUTINES_MORNING_COMPLETED) || false;

  const setMorningRoutineCompleted = (value: boolean) => {
    setSessionValue(SessionStateKeys.ROUTINES_MORNING_COMPLETED, value);
  };

  // Local state
  const [moodLogged, setMoodLogged] = useState<boolean>(false);
  const [dailyTasks, setDailyTasks] = useState<any[]>([]); // eslint-disable-line @typescript-eslint/no-explicit-any
  ...
  const fetchHabiticaTasks = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setDailyTasks(data.tasks.filter((task: { completed: boolean }) => !task.completed));
      } else {
  ...
  const handleCompleteTask = async (taskId: string) => {
    try {
      ...
      alert(`Error: ${(err as Error).message || 'Failed to complete task'}`);
    }
  };
    ...
    <div className="space-y-6">
      {morningRoutineCompleted ? (
        <div className="bg-green-900/30 border border-green-700 text-green-300 p-3 rounded-md flex items-center">
          ...
          Your Morning Routine is complete! Well done.
        </div>
      ) : (
        <>
