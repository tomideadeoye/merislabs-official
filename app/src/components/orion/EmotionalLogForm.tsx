...
import {
  ...
  useCognitiveDistortionAnalysis,
} from './cbt/CognitiveDistortionAnalysisContext';
import { CognitiveDistortionAnalysisData, LogEmotionRequestBody } from '@/types';
...
export const EmotionalLogForm: React.FC<EmotionalLogFormProps> = ({
  ...
  // Use the context to get the latest CBT analysis data
  // This hook must be called inside the CognitiveDistortionAnalysisProvider
  const { analysisData: contextCbtData } = useCognitiveDistortionAnalysis();

  // Update local cbtData state when context data changes
  useEffect(() => {
    setCbtData(contextCbtData === null ? undefined : contextCbtData);
    // Also update accompanyingThoughts if it was derived from automaticThought
    if (contextCbtData?.automaticThought && accompanyingThoughts === initialAccompanyingThoughts.current) {
      setAccompanyingThoughts(contextCbtData.automaticThought);
    }
  }, [contextCbtData, accompanyingThoughts]);

  // Ref to store the initial accompanying thoughts to prevent infinite loops
  const initialAccompanyingThoughts = React.useRef(accompanyingThoughts);
  React.useEffect(() => {
    initialAccompanyingThoughts.current = accompanyingThoughts;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: LogEmotionRequestBody = {
        ...
        physicalSensations: physicalSensations
          ? physicalSensations
              ...
              .filter(Boolean)
          : [],
        accompanyingThoughts: accompanyingThoughts || cbtData?.automaticThought, // Use cbtData from state
        copingMechanismsUsed: copingMechanisms
          ? copingMechanisms
              .split(',')
              .map((m) => m.trim())
              .filter(Boolean)
        ...
        contextualNote,
        relatedJournalSourceId,
        cognitiveDistortionAnalysis: cbtData && cbtData.automaticThought ? cbtData : undefined, // Use cbtData from state
      ...
      setFeedback({ type: 'error', message: err instanceof Error ? err.message : 'An unexpected error occurred' }); // Type narrowing
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className={`space-y-4 ${className}`}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <Label htmlFor="primaryEmotion" className="text-gray-300">
            Primary Emotion
          </Label>
          <Input
            id="primaryEmotion"
        ...
        <div>
          <Label htmlFor="intensity" className="text-gray-300">
            Intensity (1-10)
          </Label>
          <Input
            id="intensity"
      ...
      <div>
        <Label htmlFor="secondaryEmotions" className="text-gray-300">
          Secondary Emotions (comma-separated)
        </Label>
        <Input
      ...
      <div>
        <Label htmlFor="physicalSensations" className="text-gray-300">
          Physical Sensations (comma-separated)
        </Label>
        <Input
      ...
      <div>
        <Label htmlFor="accompanyingThoughts" className="text-gray-300">
        ...
        <Textarea
          ...
          onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => {
            ...
            if (cbtData) {
              // We need to trigger the context to update its state here implicitly from the user input
              // This is a more complex sync that might require a direct method on the context provider
              // For now, we will let the CBT form handle its own internal state and just pass initialData
            }
          }}
      ...
      <div>
        <Label htmlFor="copingMechanisms" className="text-gray-300">
          Coping Mechanisms Used (comma-separated)
        </Label>
        <Input
          id="copingMechanisms"
      ...
      <div>
        <Label htmlFor="contextualNote" className="text-gray-300">
          Additional Context
        </Label>
        <Textarea
          id="contextualNote"
          ...
          <AccordionContent className="pt-1 pb-0">
            <CognitiveDistortionAnalysisProvider>
              <CognitiveDistortionAnalysisForm
                initialData={cbtData} // Pass cbtData as initialData
              />
            </CognitiveDistortionAnalysisProvider>
          </AccordionContent>
        </AccordionItem>
