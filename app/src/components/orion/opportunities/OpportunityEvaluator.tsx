...
import { EvaluationOutput, OrionOpportunityDetails, OpportunityType } from '@/types/orion';
import { v4 as uuidv4 } from 'uuid';

interface OpportunityEvaluatorProps {
  className?: string;
  initialOpportunity?: OrionOpportunityDetails;
  onEvaluationComplete?: (evaluation: EvaluationOutput) => void;
}

export const OpportunityEvaluator: React.FC<OpportunityEvaluatorProps> = ({
  className,
  initialOpportunity,
  onEvaluationComplete,
}) => {
  const [title, setTitle] = useState<string>(initialOpportunity?.title || '');
  const [content, setContent] = useState<string>(initialOpportunity?.content || '');
  const [company, setCompany] = useState<string>(
    initialOpportunity?.companyOrInstitution || initialOpportunity?.company || ''
  );
  const [type, setType] = useState<OpportunityType>(initialOpportunity?.type || OpportunityType.JOB);
  const [url, setUrl] = useState<string>(initialOpportunity?.url || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const opportunityDetails: OrionOpportunityDetails = {
        id: uuidv4(),
        title: title.trim(),
  ...
  const strengths: string[] =
    evaluation?.alignmentHighlights?.map((highlight: { title: string; reasoning: string }) => highlight.title) || [];
  const gaps: string[] = evaluation?.gapAnalysis?.map((gap: { skill: string; reasoning: string }) => gap.skill) || [];
  const suggestedNextSteps: string[] = evaluation?.suggestedNextSteps?.map((step: string) => step) || [];
