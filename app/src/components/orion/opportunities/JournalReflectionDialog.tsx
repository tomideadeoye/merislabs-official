...
import { DialogHeader, DialogFooter } from '../../ui';

export const JournalReflectionDialog: React.FC = () => {
  const { isOpen, close, OrionOpportunity, actionType = 'general' } = useJournalReflectionDialogStore();
  ...
  const getPromptText = () => {
    switch (actionType) {
      case 'application_sent':
        return `Reflect on your application to ${OrionOpportunity.title} at ${OrionOpportunity.company}. How do you feel about your application? What strengths did you highlight? Any concerns?`;
      case 'interview_completed':
        return `Reflect on your interview for ${OrionOpportunity.title} at ${OrionOpportunity.company}. What went well? What could have gone better? What did you learn?`;
      case 'outreach_sent':
        return `Reflect on your outreach for ${OrionOpportunity.title} at ${OrionOpportunity.company}. What was your approach? What response do you hope for?`;
      default:
        return `Reflect on this OrionOpportunity (${OrionOpportunity.title} at ${OrionOpportunity.company}). What are your thoughts, feelings, or insights?`;
    }
  };
  ...
  const handleSaveReflection = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/orion/memory/add-memory', {
        ...
        body: JSON.stringify({
          text: reflectionText,
          sourceId: `reflection_${OrionOpportunity.id}_${Date.now()}`,
          tags: [
            'reflection',
            'opportunity_reflection',
            (OrionOpportunity.company || 'no_company').toLowerCase().replace(/\s+/g, '_'),
            actionType,
          ],
        }),
      });
      ...
      setFeedback({ type: 'error', message: error instanceof Error ? error.message : 'An unexpected error occurred.' });
    } finally {
      setIsSaving(false);
    }
  };
