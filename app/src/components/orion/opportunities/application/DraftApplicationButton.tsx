...
import type { OrionOpportunity } from '@/types/index';
...
export const DraftApplicationButton: React.FC<DraftApplicationButtonProps> = ({ orionOpportunity }) => {
  const isOpen = useOpportunityCentralStore((state: OpportunityCentralStoreType) => state.isDraftDialogOpen);
  const open = useOpportunityCentralStore((state: OpportunityCentralStoreType) => state.openDraftDialog);
  const close = useOpportunityCentralStore((state: OpportunityCentralStoreType) => state.closeDraftDialog);
  const dialogOpportunity = useOpportunityCentralStore((state: OpportunityCentralStoreType) => state.draftOpportunity);
  ...
  const handleGenerateDrafts = async () => {
    ...
    try {
      ...
      const requestBody = {
        ...
        applicant_profile: profileData.profile,
        number_of_drafts: 3,
      };
      ...
      if (err instanceof Error) {
        consolidatedLogger.error('[DraftApplicationButton][ERROR] Error generating application drafts:', err);
        setError(err.message || 'An error occurred while generating drafts');
      } else {
        consolidatedLogger.error('[DraftApplicationButton][ERROR] An unknown error occurred:', { error: err });
        setError('An unknown error occurred while generating drafts.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const copyToClipboard = (text: string) => {
    navigator.clipboard
      .writeText(text)
      .then(() => {
        consolidatedLogger.info('[DraftApplicationButton][ACTION] Copied to clipboard');
      })
      .catch((err) => {
        consolidatedLogger.error(
          '[DraftApplicationButton][ERROR] Failed to copy text:',
  ...
  return (
    <>
      <Button onClick={() => open(orionOpportunity)} className="bg-blue-600 hover:bg-blue-700">
          ...
          <DialogContent className="sm:max-w-[700px] bg-gray-800 border-gray-700 text-gray-200">
            ...
            {!drafts.length && !isLoading && !error && (
              <div className="py-6 text-center">
                ...
                <Button onClick={handleGenerateDrafts} className="bg-green-600 hover:bg-green-700">
              ...
              <div className="space-y-4">
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                  ...
                  {drafts.map((draft, index) => (
                    <TabsContent key={`draft-${index + 1}`} value={`draft-${index + 1}`} className="mt-4">
                      <div className="relative bg-gray-700 p-4 rounded-md border border-gray-600">
                        ...
                        <div className="whitespace-pre-wrap text-gray-200 pr-8">{draft}</div>
                      </div>
                ...
                <div className="pt-2 text-center">
                  <Button onClick={handleGenerateDrafts} variant="outline" className="text-gray-300 border-gray-600">
                    Regenerate Drafts
                  </Button>
                </div>
              </div>
