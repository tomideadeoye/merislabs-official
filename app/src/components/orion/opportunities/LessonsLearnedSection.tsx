  ...
  const fetchLessonsLearned = useCallback(async () => {
    ...
    try {
      ...
      if (data.success && data.results && data.results.length > 0) {
        ...
        const lessonsLearned = data.results.map(
          (item: {
            id: string;
            payload: { text?: string; source_id?: string; timestamp?: string; tags?: string[] };
            score: number;
          }) => ({
            id: item.id,
              ...
              <div key={index} className="bg-gray-700 border border-gray-600 rounded-md p-3 relative">
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  ...
                  {lesson.tags.slice(0, 3).map((tag, tagIndex) => (
                    <Badge key={tagIndex} variant="outline" className="bg-gray-600 text-gray-300">
                      {tag}
                    </Badge>
                  ))}
                </div>
