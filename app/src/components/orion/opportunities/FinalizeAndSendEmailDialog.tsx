...
import type { SendEmailParams, EmailAttachment } from '@/types/email';
...
export const FinalizeAndSendEmailDialog: React.FC = () => {
  ...
  const attachmentsToSend: EmailAttachment[] = [];
    ...
    <Dialog open={isOpen} onOpenChange={close}>
      <DialogContent className="sm:max-w-2xl bg-gray-800 border-gray-700 text-gray-200">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-400">Review and send your prepared email.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto pr-2">
          <div>
            <Label htmlFor="emailTo" className="text-gray-300">
              To:*
            </Label>
            <Input
              id="emailTo"
          ...
          <div>
            <Label htmlFor="emailSubject" className="text-gray-300">
              Subject:*
            </Label>
            <Input
              id="emailSubject"
          ...
          <div>
            <Label htmlFor="emailBody" className="text-gray-300">
              Body (HTML):*
            </Label>
            <Textarea
              id="emailBody"
              ...
              rows={15}
              className="min-h-[300px] bg-gray-700 border-gray-600 font-sans"
            />
          </div>
        ...
        {error && (
          <p className="text-sm text-red-400 flex items-center mb-2">
            <AlertTriangle className="mr-1.5 h-4 w-4" />
            {error}
          </p>
        )}
        <DialogFooter>
          <Button onClick={close} variant="outline" className="text-gray-300 border-gray-600">
            Cancel
          </Button>
          <Button
            onClick={handleSend}
            disabled={isSending || !to || !subject || !htmlBody}
