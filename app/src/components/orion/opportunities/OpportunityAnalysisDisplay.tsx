...
import { Loader2, AlertTriangle, RefreshCw, BarChartBig, CheckCircle } from 'lucide-react';
import { z } from 'zod';
import { OpportunityNotionOutputShared } from '@/types/orion';
...
export const OpportunityAnalysisDisplay: React.FC<OpportunityAnalysisDisplayProps> = ({
  ...
  const handleTriggerEvaluation = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to evaluate OrionOpportunity.');
    } finally {
      setIsLoading(false);
    }
  };
              ...
              <div className="bg-gray-800 border border-gray-700 rounded p-4">
                ...
                {evalOut.alignmentHighlights && (
                  <div className="mb-2">
                    ...
                    <ul className="list-disc ml-6 text-gray-400 text-sm">
                      {evalOut.alignmentHighlights.map((h, i: number) => (
                        <li key={i}>
                ...
                {evalOut.gapAnalysis && (
                  <div className="mb-2">
                    ...
                    <ul className="list-disc ml-6 text-gray-400 text-sm">
                      {evalOut.gapAnalysis.map((g, i: number) => (
                        <li key={i}>
