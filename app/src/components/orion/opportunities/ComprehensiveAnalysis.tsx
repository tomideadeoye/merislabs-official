...
export const ComprehensiveAnalysis: React.FC<ComprehensiveAnalysisProps> = ({ evaluation }) => {
  ...
  const {
    ...
    strengths: alignmentHighlights = [],
    gaps: gapAnalysis = [],
    suggestedCvComponents: rawSuggestedCvComponents = [],
  } = evaluation;

  const strengths: string[] = alignmentHighlights.map((item: { title: string; reasoning: string }) => item.title) || [];
  const gaps: string[] = gapAnalysis.map((item: { skill: string; reasoning: string }) => item.skill) || [];
  const cvComponentsForDisplay: string[] =
    rawSuggestedCvComponents.map((component: { component: string; reasoning: string }) => component.component) || [];
      ...
      <CardContent className="space-y-6">
        ...
        <Section title="Suggested CV Components" icon={<FileText className="text-purple-400" />}>
          <div className="flex flex-wrap gap-2">
            {cvComponentsForDisplay.map((component: string, index: number) => (
