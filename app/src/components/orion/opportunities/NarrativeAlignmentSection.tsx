...
import type { EvaluationOutput } from '@/types/orion';
  ...
  const fetchNarrativeAlignment = useCallback(async () => {
    ...
    try {
      ...
      if (memoryData.success && memoryData.results && memoryData.results.length > 0) {
        ...
        const narrativePoints = memoryData.results.map((item: { payload: { text: string } }) => item.payload.text);
        ...
        if (alignmentData.success && alignmentData.content) {
          ...
          if (highlightsMatch) {
            setNarrativeHighlights(
              highlightsMatch.map((h: string) => h.replace(/^\d\.|-\s*/, '').trim()).filter((h: string) => h.length > 0)
            );
          }
        }
      }
                ...
                <div className="space-y-2">
                  {narrativeHighlights.map((highlight, index) => (
                    <div key={index} className="flex items-start gap-2">
                      <Badge className="bg-indigo-900/30 text-indigo-300 border-indigo-700 mt-0.5">{index + 1}</Badge>
...
${narrativeHighlights.map((h, i) => `${i + 1}. ${h}`).join('\n')}
          ...
          <div className="text-center py-6">
            ...
            <Button onClick={fetchNarrativeAlignment} className="mt-2 bg-indigo-600 hover:bg-indigo-700">
