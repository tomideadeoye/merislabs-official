...
import { OpportunityType } from '@/types/orion';
...
export const OpportunityEvaluator: React.FC<OpportunityEvaluatorProps> = ({ className }) => {
  ...
  const renderRiskReward = (rrAnalysis: EvaluationOutput['riskRewardAnalysis']) => {
    ...
    const entries = Object.entries(rrAnalysis).filter(([, value]) => value);
  ...
  const rawOutputContent =
    evaluation && 'rawOutput' in evaluation && typeof evaluation.rawOutput === 'string' ? evaluation.rawOutput : null;
