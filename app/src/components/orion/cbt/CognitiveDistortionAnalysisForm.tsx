...
import { COGNITIVE_DISTORTIONS_LIST, DISTORTION_DESCRIPTIONS } from '@/lib/cbtConstants';
import { Label, Textarea, Checkbox, Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui';
import { InfoIcon } from 'lucide-react';
import type { CognitiveDistortionAnalysisData, CognitiveDistortion, CognitiveDistortionId } from '@/types';
...
const distortionOptions: (CognitiveDistortion & { selected: boolean })[] = Object.entries(
  COGNITIVE_DISTORTIONS_LIST
).map(([id, name]) => ({
  id: id as CognitiveDistortionId,
  name: name as string,
  selected: false,
  description: DISTORTION_DESCRIPTIONS[id as CognitiveDistortionId]?.description || '',
  example: DISTORTION_DESCRIPTIONS[id as CognitiveDistortionId]?.example || '',
  reframe: DISTORTION_DESCRIPTIONS[id as CognitiveDistortionId]?.reframe || '',
}));

export const CognitiveDistortionAnalysisForm: React.FC<CognitiveDistortionAnalysisFormProps> = ({ initialData }) => {
  ...
  const [distortions, setDistortions] = useState<(CognitiveDistortion & { selected: boolean })[]>(() =>
    distortionOptions.map((opt) => ({
      ...opt,
      selected: initialData?.identifiedDistortions?.includes(opt.id) || false,
    }))
  );
  ...
  const triggerChange = (
    ...
    currentDistortions: (CognitiveDistortion & { selected: boolean })[],
    currentChallenge: string,
    currentAlternative: string
  ) => {
    setAnalysisData({
      automaticThought: currentAutomaticThought,
      identifiedDistortions: currentDistortions.filter((d) => d.selected).map((d) => d.id),
  ...
  return (
    <Card className="bg-gray-800 border-gray-700/70 p-0">
      ...
      <CardContent className="space-y-4 px-4 md:px-5 pb-4">
        <div>
          <Label htmlFor="automaticThought" className="text-sm text-gray-300">
        ...
        <div>
          <Label htmlFor="challengeToThought" className="text-sm text-gray-300">
            Challenge the Thought (What evidence contradicts it?)
          </Label>
          <Textarea
        ...
        <div>
          <Label htmlFor="alternativePerspective" className="text-sm text-gray-300">
