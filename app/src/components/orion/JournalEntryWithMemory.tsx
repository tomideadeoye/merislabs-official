...
import { MemoryPayload } from '@/types/orion';
import { JournalEntryNotionInput, MemoryPoint, LogEmotionRequestBody } from '@/types/orion';

/**
 * Interface for the data structure used for journal entries across different save actions.
 */
...
export function JournalEntryWithMemory() {
  ...
  const handleSaveToNotionCallback = useCallback(
    async (dataToSave: JournalEntryData): Promise<Omit<SaveResult, 'serviceName'>> => {
      const notionPayload: JournalEntryNotionInput = {
        ...
        date: dataToSave.date instanceof Date ? dataToSave.date.toISOString() : new Date(dataToSave.date).toISOString(), // Ensure date is ISO string
      ...
      try {
        ...
        return { success: false, error: errorMessage || 'Error saving to Notion.' };
      }
    },
    []
  );

  const handleSaveToQdrantCallback = useCallback(
    async (dataToSave: JournalEntryData): Promise<Omit<SaveResult, 'serviceName'>> => {
      ...
      try {
        ...
        const memoryPayload: MemoryPayload = {
          ...
          type: 'journal_entry',
          title: title || content.substring(0, 50) + '...', // Ensure title is always a string
        ...
        if (upsertData.success) {
          return {
            success: true,
            message: 'Saved to Qdrant (Vector Memory).',
            data: { sourceId: sourceId, qdrantId: memoryPoint.id },
          };
        }
        ...
        return { success: false, error: errorMessage || 'Error saving to Qdrant.' };
      }
    },
    []
  );

  const handleSaveToSQLiteCallback = useCallback(
    async (dataToSave: JournalEntryData): Promise<Omit<SaveResult, 'serviceName'>> => {
      const { content, title, date, metadata } = dataToSave;
      ...
      try {
        ...
        return { success: false, error: errorMessage || 'Error saving to SQLite.' };
      }
    },
    []
  );
  ...
  const handleCopyToClipboardCallback = useCallback(
    async (dataToSave: JournalEntryData): Promise<Omit<SaveResult, 'serviceName'>> => {
      ...
      try {
        ...
        return {
          success: false,
          error: errorMessage || 'Failed to copy to clipboard. Ensure you are in a secure context.',
        };
      }
    },
    []
  ...
  const journalDataForButton: JournalEntryData = {
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="mood" className="text-gray-400">
                Mood (Optional)
              </Label>
              <Input
                id="mood"
            ...
            <div>
              <Label htmlFor="tagInput" className="text-gray-400">
                Tags (Optional)
              </Label>
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={!tagInput.trim() || isOverallSubmitting}
