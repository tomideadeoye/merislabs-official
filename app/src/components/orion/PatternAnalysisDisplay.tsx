...
import { Loader2, Lightbulb, Brain, Info, ListTodo } from 'lucide-react';
import type { IdentifiedPattern } from '@repo/shared';
...
export const PatternAnalysisDisplay: React.FC<PatternAnalysisDisplayProps> = ({ className }) => {
  ...
  // const [selectedInsight, setSelectedInsight] = useState<string>(""); // Removed
  // const [selectedTheme, setSelectedTheme] = useState<string>(""); // Removed
  ...
  const handleCreateTask = (insight: string, theme: string) => {
    // setSelectedInsight(insight); // Removed
    // setSelectedTheme(theme); // Removed
    openDialog({
      initialTaskText: insight,
      initialTaskNotes: `From Pattern Analysis: ${theme}`,
      sourceModule: 'Pattern Tracker',
      sourceReferenceId: theme,
    });
  };
      ...
      <Card className="bg-gray-800 border-gray-700">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="types" className="text-gray-300">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="customQuery" className="text-gray-300">
          ...
          {patterns.map((pattern, index) => (
            <Card key={index} className="bg-gray-800 border-gray-700">
              <CardHeader className="pb-2">
                ...
                {pattern.sentiment && (
                  <CardDescription className="text-gray-400">Sentiment: {pattern.sentiment}</CardDescription>
                )}
              </CardHeader>
                ...
                {pattern.supportingMemoryIds && pattern.supportingMemoryIds.length > 0 && (
                  <div className="text-sm text-gray-400">
                    ...
                    <ul className="list-disc list-inside">
                      {pattern.supportingMemoryIds.slice(0, 5).map((id: string, idx: number) => (
                        <li key={idx}>{id}</li>
                      ))}
