...
import { IdeaStatus, Idea } from '@repo/shared/types/ideas';
...
export const IdeaList: React.FC<IdeaListProps> = ({ className }) => {
  ...
  const applyFilters = useCallback(() => {
    ...
    // Apply tag filter
    if (tagFilter) {
      result = result.filter((idea) => idea.tags?.some((tag) => tag.toLowerCase().includes(tagFilter.toLowerCase())));
    }
  ...
  return (
    <div className={className}>
      <div className="flex justify-between items-center mb-4">
        ...
        <Button variant="outline" size="sm" onClick={handleRefresh} className="text-gray-300 border-gray-600">
              ...
              <Card className="bg-gray-750 border-gray-700 hover:bg-gray-700 transition-colors cursor-pointer">
                <CardContent className="p-4">
                  <div className="flex justify-between items-start">
                    ...
                    <Badge
                      className={`${status && statusConfig[status as IdeaStatus] ? statusConfig[status as IdeaStatus].color : 'bg-gray-400'} text-white`}
                    >
                      {status && statusConfig[status as IdeaStatus]
                        ? statusConfig[status as IdeaStatus].label
                        : 'Unknown'}
                    </Badge>
                  </div>

                  {idea.description && <p className="text-gray-400 text-sm mt-1 line-clamp-2">{idea.description}</p>}
