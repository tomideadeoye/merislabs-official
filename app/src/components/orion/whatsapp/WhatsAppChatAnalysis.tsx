...
export const WhatsAppChatAnalysis: React.FC<WhatsAppChatAnalysisProps> = ({ analysisData, className }) => {
                  ...
                  <div className="space-y-2">
                    {chat.participants.map((participant: string) => {
                      ...
                      const maxAvgLength = Math.max(
                        ...(Object.values(basicStats.avgMessageLengthByParticipant) as number[])
                      );
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {chat.participants.map((participant: string) => {
                      ...
                      const totalMediaCount = Object.values(basicStats.mediaCountByParticipant).reduce(
                    ...
                    {Object.entries(basicStats.messagesByHour).map(([hour, count]: [string, any]) => {
                      ...
                      return (
                        <div key={hour} className="flex flex-col items-center">
                          <div className="w-full bg-cyan-500 rounded-t-sm" style={{ height: `${height}px` }}></div>
            ...
            <CardContent>
              {insights ? (
                <div className="space-y-6">
                  ...
                  {insights.sentimentAnalysis && insights.sentimentAnalysis.length > 0 && (
                    <div>
                      <h3 className="text-md font-medium text-green-400 mb-2">Sentiment Analysis</h3>
                      <ul className="list-disc list-inside space-y-1">
                        {insights.sentimentAnalysis.map((sentiment: string, index: number) => (
                          <li key={index} className="text-gray-300">
                            {sentiment}
                          </li>
                        ))}
                  ...
                  {insights.keyTopics && insights.keyTopics.length > 0 && (
                    <div>
                      <h3 className="text-md font-medium text-purple-400 mb-2">Key Topics</h3>
                      <ul className="list-disc list-inside space-y-1">
                  ...
                  {insights.actionableItems && insights.actionableItems.length > 0 && (
                    <div>
                      <h3 className="text-md font-medium text-amber-400 mb-2">Actionable Items</h3>
                      <ul className="list-disc list-inside space-y-1">
                        {insights.actionableItems.map((item: string, index: number) => (
                          <li key={index} className="text-gray-300">
                            {item}
                          </li>
                        ))}
                ...
                <p className="text-gray-400 text-center py-4">
                  No AI insights available yet. Upload a chat to generate insights.
                </p>
              )}
            </CardContent>
          </Card>
