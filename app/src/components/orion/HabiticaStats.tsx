...
import { Card, CardContent, Progress } from '@/ui/components/ui';
...
export const HabiticaStats: React.FC<HabiticaStatsProps> = ({ className }) => {
  ...
  const fetchStats = async () => {
    ...
    try {
      ...
      if (data.success) {
        setStats(data.userStats as HabiticaUserStats);
      } else {
      ...
      console.error('Error fetching Habitica stats:', errorMessage);
  ...
  const { hp, maxHealth, mp, maxMP, exp, toNextLevel, lvl, gp } = stats;
  const username = 'Habitican';
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm text-gray-300">
                {Math.round(exp)}/{toNextLevel}
              </span>
            </div>
            <Progress
              value={(exp / toNextLevel) * 100}
