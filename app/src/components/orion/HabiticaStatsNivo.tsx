...
interface HabiticaStat {
  label: string;
  value: number;
  [key: string]: string | number;
}

interface HabiticaTaskCompletionStat {
  id: string;
  label: string;
  value: number;
}

interface HabiticaStatsData {
  stats: {
    hp: number;
    exp: number;
    mp: number;
...
interface HabiticaTask {
  completed: boolean;
  // Add other common task properties like id, text, type if relevant and used
}

const NivoBarChart = ({ data }: { data: HabiticaStat[] }) => (
  <ResponsiveBar
    data={data}
    keys={['value']}
    indexBy="label"
...
const NivoPieChart = ({ data }: { data: HabiticaTaskCompletionStat[] }) => (
  <ResponsivePie
    data={data}
...
export const HabiticaStatsNivo: React.FC<HabiticaStatsNivoProps> = ({ className }) => {
  ...
  const [stats, setStats] = useState<HabiticaStatsData | null>(null);
  ...
  const initialHabiticaStatsString = getSessionValue(SessionStateKeys.HABITICA_STATS);
