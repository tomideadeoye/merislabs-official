...
import { Button, Textarea, Input, Label, Checkbox } from '@/components/ui';
...
interface AskQuestionPayload {
  requestType: typeof ASK_QUESTION_REQUEST_TYPE;
...
export const AskQuestionForm: React.FC = () => {
  const { getSessionValue, setSessionValue } = useSessionStateStore();
  const [question, setQuestion] = useState<string>(getSessionValue(SessionStateKeys.ASK_Q_INPUT) || '');
  const [answer, setAnswer] = useState<string>(getSessionValue(SessionStateKeys.ASK_Q_ANSWER) || '');
  const [isProcessing, setIsProcessing] = useState<boolean>(
    getSessionValue(SessionStateKeys.ASK_Q_PROCESSING) || false
  );
  ...
  useEffect(() => {
    setSessionValue(SessionStateKeys.ASK_Q_INPUT, question);
  }, [question, setSessionValue]);

  useEffect(() => {
    setSessionValue(SessionStateKeys.ASK_Q_ANSWER, answer);
  }, [answer, setSessionValue]);

  useEffect(() => {
    setSessionValue(SessionStateKeys.ASK_Q_PROCESSING, isProcessing);
  }, [isProcessing, setSessionValue]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (memoryTags.trim()) {
        payload.memorySourceTags = memoryTags
          .split(',')
          .map((tag) => tag.trim())
          .filter(Boolean);
        setFiltersApplied(true);
  ...
  const handleTypeToggle = (typeId: string) => {
    setSelectedMemoryTypes((prev) => (prev.includes(typeId) ? prev.filter((id) => id !== typeId) : [...prev, typeId]));
  };
    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium text-gray-300">
                Memory Filters
                {filtersApplied && (
                  <span className="ml-2 text-xs bg-blue-600 text-white px-1.5 py-0.5 rounded-full">Active</span>
                )}
              </span>
            </div>
            {showFilters ? (
          ...
          {showFilters && (
            <div className="p-3 border-t border-gray-700 bg-gray-750">
              <div className="space-y-3">
                <div>
                  <Label className="text-sm text-gray-400 mb-2 block">Memory Types</Label>
                  <div className="grid grid-cols-2 gap-2">
                    {MEMORY_TYPES.map((type) => (
                      <div key={type.id} className="flex items-center space-x-2">
                        ...
                        />
                        <Label htmlFor={`type-${type.id}`} className="text-sm text-gray-300 cursor-pointer">
                          {type.label}
                        </Label>
                      </div>
                    ))}
                ...
                <div>
                  ...
                  <Input
                    id="memoryTags"
                    value={memoryTags}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setMemoryTags(e.target.value)}
        ...
        <div className="flex justify-between items-center">
          <Button type="submit" disabled={isButtonDisabled} className="bg-blue-600 hover:bg-blue-700">
            {isProcessing ? (
              <>
