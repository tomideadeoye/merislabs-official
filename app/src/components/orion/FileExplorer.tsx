...
import { Button, Input, Card, CardContent, CardHeader, CardTitle } from '@/ui/components/ui';
...
export const FileExplorer: React.FC<FileExplorerProps> = ({ className }) => {
        ...
        {configuredDirs.map((dir) => (
          <div key={dir} className="space-y-1">
            ...
            >
              ...
              <span className="text-xs text-gray-500 ml-2 truncate" title={dir}>
                {dir}
              </span>
            </div>
                ...
                {isLoading && currentPath === dir ? (
                  ...
                  contents.map((item) => (
                    <div
                      ...
                      onClick={() =>
                        item.type === 'directory' ? handleDirectoryClick(item.path) : handleFileClick(item.path)
                      }
                    >
                      {item.type === 'directory' ? (
                        <>
  ...
  return (
    <Card className={`bg-gray-800 border-gray-700 ${className}`}>
      ...
      <CardContent>{renderFileTree()}</CardContent>
    </Card>
  );
};
