...
import { Persona, useSessionState, SessionStateKeys } from '@/index';
import { Label } from '@radix-ui/react-label';
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from '@radix-ui/react-select';
import { Send, AlertTriangle, Loader2 } from 'lucide-react';
import React, { useState } from 'react';
import { Card, CardHeader, CardTitle, CardDescription, CardContent, Textarea, Input, Button } from '../ui';
...
export const OutreachForm: React.FC<OutreachFormProps> = ({ persona }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!String(opportunityDetails).trim() || !String(goal).trim()) {
      setError('OrionOpportunity details and goal are required.');
      return;
    }
    ...
    <Card className="bg-gray-800 border-gray-700">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <Label htmlFor="opportunityDetails" className="text-gray-300">
              {' '}
            ...
            <Textarea
              ...
              placeholder="Describe the OrionOpportunity or context for this outreach"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="communicationType" className="text-gray-300">
                {' '}
                Communication Type{' '}
              </Label>
              <Select
                value={communicationType || 'email'}
                onValueChange={(value: string) => setCommunicationType(value)}
                disabled={isGenerating}
              >
          ...
          <Button
            type="submit"
            disabled={isGenerating || !String(opportunityDetails).trim() || !String(goal).trim()}
