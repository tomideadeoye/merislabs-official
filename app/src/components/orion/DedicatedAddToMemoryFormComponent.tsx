...
export const DedicatedAddToMemoryFormComponent: React.FC<DedicatedAddToMemoryFormProps> = ({
  ...
  const text = selectSessionValue<string>(SessionStateKeys.ATM_PASTED_TEXT) || initialText;
  const sourceId = selectSessionValue<string>(SessionStateKeys.ATM_SOURCE_ID) || initialSourceId;
  const tags = selectSessionValue<string>(SessionStateKeys.ATM_TAGS_INPUT) || initialTags;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (!embeddingData.success || !embeddingData.embeddings || embeddingData.embeddings.length === 0) {
          ...
          if (embeddingData.results && Array.isArray(embeddingData.results)) {
            ...
            const errorList = failed
              .map((f: any) => `• "${f.text.substring(0, 50)}..." — ${f.error || 'Unknown error'}`)
              .join('\n');
            throw new Error(`Failed to generate embeddings for the following text(s):\n${errorList}`);
          }
      ...
      setSessionValue(SessionStateKeys.ATM_PASTED_TEXT, '');
      setSessionValue(SessionStateKeys.ATM_SOURCE_ID, '');
      setSessionValue(SessionStateKeys.ATM_TAGS_INPUT, '');
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <Textarea
          id="text"
          value={text || ''}
          onChange={(e) => setSessionValue(SessionStateKeys.ATM_PASTED_TEXT, e.target.value)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <Input
            ...
            type="text"
            value={sourceId || ''}
            onChange={(e) => setSessionValue(SessionStateKeys.ATM_SOURCE_ID, e.target.value)}
        ...
        <div>
          ...
          <Input
            ...
            type="text"
            value={tags || ''}
            onChange={(e) => setSessionValue(SessionStateKeys.ATM_TAGS_INPUT, e.target.value)}
      ...
      <div className="mb-4">
        <Label className="block text-sm font-medium text-gray-300 mb-1">Save to:</Label>
        ...
        <div
          className={`mt-4 p-3 rounded-md text-sm flex items-start ${
            feedback.type === 'success'
              ? 'bg-green-800/30 border border-green-600 text-green-300'
