...
import { Input, Button, Card, CardContent, CardHeader, CardTitle, Badge } from '@/ui/components/ui';
import { Search, Tag, Calendar } from 'lucide-react';
import { useMemory } from '@/app/index';
...
export function MemorySearch() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        {results.map((result: ScoredMemoryPoint) => (
          <Card key={result.id} className="bg-gray-800 border-gray-700">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm text-blue-400 flex items-center">
                ...
                {result.payload.type}
                <span className="ml-auto text-xs text-gray-400">Score: {result.score.toFixed(2)}</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-gray-300 whitespace-pre-wrap">{result.payload.text}</p>
