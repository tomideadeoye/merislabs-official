...
export const StatusUpdateButton: React.FC<StatusUpdateButtonProps> = ({
  ...
  const currentStatusLabel =
    statusGroups.flatMap((group) => group.statuses).find((status) => status.value === currentStatus)?.label ||
    'Unknown Status';
            ...
            <React.Fragment key={group.label}>
              ...
              <DropdownMenuGroup>
                ...
                {group.statuses.map((status) => (
                  ...
                  >
                    {status.label}
                    {status.value === currentStatus && <span className="ml-auto text-xs text-gray-400">(Current)</span>}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuGroup>
            </React.Fragment>
