...
export function useMemory() {
  ...
  const search = useCallback(async (query: string, options?: MemorySearchOptions) => {
    ...
    try {
      ...
      setError((err instanceof Error ? err.message : String(err)) || 'Error searching memory');
      return [];
    } finally {
      setIsLoading(false);
    }
  ...
  const add = useCallback(
    async (
      ...
      try {
        const response = await addMemory(text, source_id, type, tags, additionalFields);
        ...
        setError((err instanceof Error ? err.message : String(err)) || 'Error adding memory');
        return false;
      } finally {
        setIsLoading(false);
      }
  ...
  const findByType = useCallback(async (type: string, limit: number = 10) => {
    ...
    try {
      ...
      setError((err instanceof Error ? err.message : String(err)) || 'Error finding memories by type');
      return [];
    } finally {
      setIsLoading(false);
    }
  ...
  const findByTag = useCallback(async (tag: string, limit: number = 10) => {
    ...
    try {
      ...
      setError((err instanceof Error ? err.message : String(err)) || 'Error finding memories by tag');
      return [];
    } finally {
      setIsLoading(false);
    }
