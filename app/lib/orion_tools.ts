...
// Try to import from openai directly, fallback to local type if not available
// @ts-expect-error: openai/resources/chat/completions may not exist in all versions
import type { ChatCompletionTool } from 'openai/resources/chat/completions';
// Fallback type if import fails
// type ChatCompletionTool = any;

// Fallback type for ChatCompletionTool if not available from openai
...
declare global {
  interface Window {
    mcp_sequentialThinking?: (args: {
      thought: string;
      nextThoughtNeeded?: boolean;
      thoughtNumber?: number;
      totalThoughts?: number;
    }) => Promise<{ thought: string; nextThoughtNeeded: boolean; thoughtNumber: number; totalThoughts: number }>;
  }
}
...
export const searchOrionMemoryTool: ChatCompletionTool = {
  ...
  function: {
    ...
    description:
      "Searches Tomide's personal Orion memory (Qdrant vector store) for relevant information based on a natural language query and optional filters. Use this to recall past journal entries, notes, ideas, or other stored knowledge.",
    parameters: {
      type: 'object',
      properties: {
        queryText: {
          type: 'string',
          description:
            'The natural language query to search for in memory. Should be descriptive of the information sought.',
        },
        memorySourceTypes: {
          type: 'array',
          items: { type: 'string' },
          description:
            "Optional. List of memory types to filter by (e.g., 'journal_entry', 'local_document_txt', 'idea_incubator_note', 'opportunity_evaluation'). Only include if user specifies a source or if context implies one.",
        },
        memorySourceTags: {
          type: 'array',
          items: { type: 'string' },
          description:
            "Optional. List of tags to filter memories by (e.g., 'career', 'project_phoenix', 'fintech'). Only include if user mentions specific tags or topics.",
        },
        limit: {
          type: 'integer',
...
export const createHabiticaTodoTool: ChatCompletionTool = {
  ...
  function: {
    ...
    description:
      "Creates a new To-Do task in Tomide's Habitica account. Use this when an actionable item is identified.",
    parameters: {
      type: 'object',
      properties: {
        taskText: {
...
export const AVAILABLE_ORION_TOOLS: ChatCompletionTool[] = [searchOrionMemoryTool, createHabiticaTodoTool];
...
export async function callSequentialThinking({
  ...
  try {
    ...
    throw new Error(
      (err instanceof Error ? err.message : String(err)) || 'Sequential Thinking MCP tool is not available.'
    );
  }
}
