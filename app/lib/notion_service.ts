...
if (NOTION_API_KEY) {
  ...
  logger.warn('[NOTION_SERVICE] NOTION_API_KEY not found, Notion integration will be disabled.');
}

logger.debug(`[NOTION_SERVICE] Imported NOTION_API_KEY: ${NOTION_API_KEY ? 'Loaded' : '[NOT SET]'}`);
logger.debug(`[NOTION_SERVICE] Imported NOTION_DATABASE_ID: ${NOTION_DATABASE_ID || '[NOT SET]'}`);
logger.debug(`[NOTION_SERVICE] Imported NOTION_CV_DATABASE_ID: ${NOTION_CV_DATABASE_ID || '[NOT SET]'}`);
 ...
 */
function mapNotionStatusToOpportunityStatus(statusString: string | null | undefined): OpportunityStatus | undefined {
  if (!statusString) return undefined;
  if (Object.values(OpportunityStatus).includes(statusString as OpportunityStatus)) {
    return statusString as OpportunityStatus;
  }
  logger.warn(`[NOTION_SERVICE] Unknown OpportunityStatus encountered: ${statusString}`);
  return undefined;
}
 ...
 */
function mapNotionTypeToOpportunityType(typeString: string | null | undefined): OpportunityType | undefined {
  ...
  logger.warn(`[NOTION_SERVICE] Unknown OpportunityType encountered: ${typeString}`);
  return undefined;
}
...
function mapNotionPriorityToOpportunityPriority(
  ...
  if (Object.values(OpportunityPriority).includes(priorityString as OpportunityPriority)) {
    return priorityString as OpportunityPriority;
  }
  logger.warn(`[NOTION_SERVICE] Unknown OpportunityPriority encountered: ${priorityString}`);
  return undefined;
}
 ...
 */
function getDateString(date: NotionDateProperty | null): string | null {
 ...
 */
function getPropertyValue(properties: PageObjectResponse['properties'], propertyName: string, type: string): unknown {
  ...
  switch (type) {
    case 'title':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a title type.`, {
    ...
    case 'rich_text':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a rich_text type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'url':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a url type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'select':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a select type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'multi_select':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a multi_select type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'date':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a date type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'number':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a number type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'checkbox':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a checkbox type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'email':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not an email type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'phone_number':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a phone_number type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'files':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a files type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'people':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a people type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'relation':
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a relation type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    case 'formula':
      if (prop.type === 'formula') {
        if (!prop.formula) {
          logger.debug('[NOTION_SERVICE] Formula property value is null or undefined.', { propertyName, type });
          return undefined;
        }
        ...
        switch ((prop.formula as { type: string }).type) {
          ...
          default:
            logger.warn(`[NOTION_SERVICE] Unknown formula property type: ${(prop.formula as { type: string }).type}`, {
              propertyName,
              type,
            });
            return undefined;
      ...
      logger.warn(`[NOTION_SERVICE] Property ${propertyName} is not a formula type.`, {
        actualType: prop.type,
        expectedType: type,
      });
      return undefined;
    default:
      logger.warn(`[NOTION_SERVICE] Unknown property type encountered: ${type}`, { propertyName, properties });
      return undefined;
  }
}

export async function createOpportunityInNotion(data: Partial<OrionOpportunity>) {
  logger.info('[NOTION][createOpportunityInNotion][START]', { data });
  if (!notion || !NOTION_DATABASE_ID) {
    logger.warn('[NOTION][createOpportunityInNotion][WARN] Notion client or database ID not configured.');
    return null;
  }
  try {
...
export async function listOpportunitiesFromNotion(
  ...
  try {
    ...
    const opportunities: OpportunityNotionOutputShared[] = pageResults.map((page) => {
      ...
      const company = getPropertyValue(properties, 'companyOrInstitution', 'rich_text') as string;

      const statusString = getPropertyValue(properties, 'Status', 'select') as string | null | undefined;
      const status = mapNotionStatusToOpportunityStatus(statusString);

      const typeString = getPropertyValue(properties, 'Type', 'select') as string | null | undefined;
      const type = mapNotionTypeToOpportunityType(typeString);

      const url = getPropertyValue(properties, 'URL', 'url') as string | null;
      const content = getPropertyValue(properties, 'Content', 'rich_text') as string;
      const tags = getPropertyValue(properties, 'Tags', 'multi_select') as string[] | null;
      const dateIdentified = getPropertyValue(properties, 'Date', 'date') as string | null;
      const lastEditedTime = page.last_edited_time; // last_edited_time is directly on PageObjectResponse

      const priorityString = getPropertyValue(properties, 'Priority', 'select') as string | null | undefined;
      const priority = mapNotionPriorityToOpportunityPriority(priorityString);

      const evaluationOutput = getPropertyValue(properties, 'Evaluation Output', 'rich_text') as string | null;
      const webResearchContext = getPropertyValue(properties, 'Web Research Context', 'rich_text') as string | null;
      const contentType = getPropertyValue(properties, 'Content Type', 'select') as string | null;
      const notes = getPropertyValue(properties, 'Notes', 'rich_text') as string | null;
      const pros = getPropertyValue(properties, 'Pros', 'multi_select') as string[] | null;
      const cons = getPropertyValue(properties, 'Cons', 'multi_select') as string[] | null;
      const missingSkills = getPropertyValue(properties, 'Missing Skills', 'multi_select') as string[] | null;
      const relatedEvaluationId = getPropertyValue(properties, 'Related Evaluation ID', 'rich_text') as string | null;
      const lastStatusUpdate = getPropertyValue(properties, 'Last Status Update', 'date') as string | null;
      const deadline = getPropertyValue(properties, 'Deadline', 'date') as string | null;
      const location = getPropertyValue(properties, 'Location', 'rich_text') as string | null;
      const contact = getPropertyValue(properties, 'Contact', 'rich_text') as string | null;
      const position = getPropertyValue(properties, 'Position', 'rich_text') as string | null;
...
export async function getOpportunityDetails(pageId: string) {
  ...
  if (!notion) {
    logger.warn('[NOTION][getOpportunityDetails][WARN] Notion client not configured.');
    return null;
  }
  ...
  try {
    ...
    if (!('properties' in page)) {
      logger.warn('[NOTION][getOpportunityDetails][WARN] Retrieved page does not have properties.', { pageId });
      return null;
    }
...
export async function getOpportunityContent(pageId: string) {
  ...
  if (!notion) {
    logger.warn('[NOTION][getOpportunityContent][WARN] Notion client not configured.');
    return null;
  }
  ...
  try {
    ...
    for (const block of blockResponse.results) {
      if (block.object !== 'block') {
        logger.warn('[NOTION][getOpportunityContent][WARN] Skipping non-block object.', { block });
        continue;
      }
      const typedBlock = block as BlockObjectResponse; // Explicitly cast to BlockObjectResponse

      if (typedBlock.type === 'paragraph' && 'paragraph' in typedBlock && typedBlock.paragraph.rich_text) {
        content += getRichTextContent(typedBlock.paragraph.rich_text) + '\n';
      } else if (typedBlock.type === 'heading_1' && 'heading_1' in typedBlock && typedBlock.heading_1.rich_text) {
        content += '# ' + getRichTextContent(typedBlock.heading_1.rich_text) + '\n';
      } else if (typedBlock.type === 'heading_2' && 'heading_2' in typedBlock && typedBlock.heading_2.rich_text) {
        content += '## ' + getRichTextContent(typedBlock.heading_2.rich_text) + '\n';
      } else if (typedBlock.type === 'heading_3' && 'heading_3' in typedBlock && typedBlock.heading_3.rich_text) {
        content += '### ' + getRichTextContent(typedBlock.heading_3.rich_text) + '\n';
      } else if (
        typedBlock.type === 'bulleted_list_item' &&
        'bulleted_list_item' in typedBlock &&
        typedBlock.bulleted_list_item.rich_text
      ) {
        content += '* ' + getRichTextContent(typedBlock.bulleted_list_item.rich_text) + '\n';
      } else if (
        typedBlock.type === 'numbered_list_item' &&
        'numbered_list_item' in typedBlock &&
        typedBlock.numbered_list_item.rich_text
      ) {
        content += '1. ' + getRichTextContent(typedBlock.numbered_list_item.rich_text) + '\n';
      } else if (typedBlock.type === 'to_do' && 'to_do' in typedBlock && typedBlock.to_do.rich_text) {
        content += '- [ ] ' + getRichTextContent(typedBlock.to_do.rich_text) + '\n';
      } else if (typedBlock.type === 'code' && 'code' in typedBlock && typedBlock.code.rich_text) {
        content += '```\n' + getRichTextContent(typedBlock.code.rich_text) + '\n```\n';
      } else if (typedBlock.type === 'quote' && 'quote' in typedBlock && typedBlock.quote.rich_text) {
        content += '> ' + getRichTextContent(typedBlock.quote.rich_text) + '\n';
      } else if (typedBlock.type === 'callout' && 'callout' in typedBlock && typedBlock.callout.rich_text) {
        content += '> ' + getRichTextContent(typedBlock.callout.rich_text) + '\n';
      } else {
...
export async function updateOpportunityStatus(pageId: string, status: string) {
  ...
  if (!notion) {
    logger.warn('[NOTION][updateOpportunityStatus][WARN] Notion client not configured.');
    return null;
  }
...
export async function addStakeholderToOpportunity(
  ...
  if (!notion || !NOTION_DATABASE_ID) {
    ...
    logger.warn('[NOTION][addStakeholderToOpportunity][WARN] Notion client or database ID not configured.');
    return null;
  }
  ...
  try {
    ...
    if (!('properties' in currentPage)) {
      logger.warn('[NOTION][addStakeholderToOpportunity][WARN] Current page does not have properties.', { pageId });
      return null;
    }
    ...
    if (existingStakeholdersProp && existingStakeholdersProp.type === 'relation') {
    ...
    const updatedRelationIds = [...existingRelationIds, { id: newStakeholderId }];
...
export async function saveOutreachToNotion(
  ...
  if (!notion) {
    logger.warn('[NOTION][saveOutreachToNotion][WARN] Notion client not configured.');
    return null;
  }
...
export async function getCVComponentsFromNotion(): Promise<CVComponent[]> {
  ...
  try {
    ...
    const cvComponents: CVComponent[] = pageResults.map((page) => {
      ...
      const content = getPropertyValue(properties, 'Content', 'rich_text') as string;
      const type = String((getPropertyValue(properties, 'Type', 'select') as string | null | undefined) ?? 'Unknown'); // Using String() and nullish coalescing
      const keywords = getPropertyValue(properties, 'Keywords', 'multi_select') as string[] | null;
      const uniqueId = getPropertyValue(properties, 'Unique ID', 'rich_text') as string | null;
      const notionPageId = page.id;
      const componentName = getPropertyValue(properties, 'Component Name', 'rich_text') as string | null; // Assuming rich_text or title
      const componentType = String(
        (getPropertyValue(properties, 'Component Type', 'select') as string | null | undefined) ?? 'Unknown'
      ); // Assuming select, providing fallback
      const contentPrimary = getPropertyValue(properties, 'Content Primary', 'rich_text') as string | null;
      const startDate = getPropertyValue(properties, 'Start Date', 'date') as string | null;
      const endDate = getPropertyValue(properties, 'End Date', 'date') as string | null;
...
export async function fetchOpportunityByIdFromNotion(opportunityId: string): Promise<{
  ...
  if (!notion) {
    logger.warn('[NOTION][fetchOpportunityByIdFromNotion][WARN] Notion client not configured.');
  ...
  try {
    ...
    if (!('properties' in response)) {
      logger.warn('[NOTION][fetchOpportunityByIdFromNotion][WARN] Retrieved page does not have properties.', {
        opportunityId,
      });
...
export async function updateNotionOpportunity(
  ...
  if (!notion) {
    logger.warn('[NOTION][updateNotionOpportunity][WARN] Notion client not configured.');
    return { success: false, error: 'Notion client not configured' };
  }
    ...
    const journalEntries = response.results
      .map((item) => {
        ...
        if (!('properties' in item)) {
          ...
          logger.warn('[NOTION][getJournalEntriesFromNotion][WARN] Page does not have properties.', {
            pageId: item.id,
          });
        ...
        const content = getPropertyValue(properties, 'Content', 'rich_text') as string;
        const reflectionId = getPropertyValue(properties, 'Reflection ID', 'rich_text') as string | null;
        const original_entry_id = getPropertyValue(properties, 'Original Entry ID', 'rich_text') as string | null;
        const tags = getPropertyValue(properties, 'Tags', 'multi_select') as string[] | null;
        const mood = getPropertyValue(properties, 'Mood', 'select') as string | null;
        const createdAt = page.created_time; // Directly from page, not properties
        const updatedAt = page.last_edited_time; // Directly from page, not properties
  ...
  try {
    const response = await notion.pages.create({
      ...
      properties: {
        ...
        Date: {
          date: {
            start: data.date instanceof Date ? data.date.toISOString() : data.date,
          },
...
export async function fetchContactsFromNotion(): Promise<Contact[]> {
  ...
  try {
    ...
    const contacts = response.results
      .map((item) => {
        if (!('properties' in item)) {
          logger.warn('[NOTION][fetchContactsFromNotion][WARN] Page does not have properties.', { pageId: item.id });
          return null; // Return null for invalid pages
        }
        const page = item as PageObjectResponse; // Cast to PageObjectResponse
        const properties = page.properties;
        ...
        const name = getPropertyValue(properties, 'Name', 'title') as string;
        const email = getPropertyValue(properties, 'Email', 'email') as string | null;
        const role = getPropertyValue(properties, 'Role', 'rich_text') as string | null;
        const company = getPropertyValue(properties, 'Company', 'rich_text') as string | null;
        const linkedinUrl = getPropertyValue(properties, 'LinkedIn URL', 'url') as string | null;
...
export async function updateNotionDatabaseSchema(
  databaseId: string,
  properties: Record<string, unknown> // Changed to unknown
  ...
  if (!notion) {
    logger.warn('[NOTION][updateNotionDatabaseSchema][WARN] Notion client not configured.');
    return { success: false, error: 'Notion client not configured' };
  }
