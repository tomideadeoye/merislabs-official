...
export const LitigationSlide: React.FC<{ data: LitigationData }> = ({ data }) => {
    return (
        <div className="w-full h-full relative overflow-hidden bg-[#0A0A0A] font-sans">
            ...
            {/* Content Core */}
            <div className="absolute top-[160px] bottom-16 left-16 right-16 z-10 grid grid-cols-12 gap-6">

                {/* Summary Column */}
                <div className="col-span-5 flex flex-col h-full overflow-hidden">
                    <div className="bg-[#111] border border-white/5 rounded-sm p-7 h-full relative group hover:border-red-600/30 transition-colors flex flex-col">
                        ...
                        <div className="text-gray-400 text-[13px] leading-relaxed text-justify space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                            {data.summary.split('\n').map((para, i) => (
                ...
                <div className="col-span-7 flex flex-col space-y-6 h-full overflow-hidden">
                    {/* Status Box */}
                    <div className="bg-[#161616] border border-white/5 rounded-sm p-7 flex-grow relative overflow-hidden flex flex-col">
                        ...
                        <h3 className="text-white text-[10px] font-black tracking-[0.4em] uppercase mb-4 shrink-0 opacity-40">Status Update</h3>
                        <div className="text-white/80 text-[13px] leading-relaxed space-y-3 overflow-y-auto pr-2 custom-scrollbar">
                            {data.status.split('\n').map((para, i) => (
                                <p key={i} className={i > 0 ? "mt-3" : ""}>{para}</p>
                            ))}
                        </div>
                    ...
                    {data.nextSteps && (
                        <div className="bg-red-600 p-7 rounded-sm shadow-2xl relative overflow-hidden group shrink-0">
                            <div className="absolute inset-0 bg-black/10 group-hover:bg-black/0 transition-colors" />
                            <h3 className="text-white text-[10px] font-black tracking-[0.4em] uppercase mb-3 relative z-10">Strategic Next Steps</h3>
                            <p className="text-white font-bold text-[13px] leading-relaxed relative z-10">
                                {data.nextSteps}
                            </p>
                            {/* Decorative icon */}
                            <div className="absolute bottom-[-15px] right-[-5px] text-white/10 text-7xl font-black italic select-none">NEXT</div>
                        </div>
                    )}
                </div>
            </div>
            ...
            {/* Pagination / ID */}
            <div className="absolute bottom-10 right-16 flex items-center space-x-4 opacity-10">
                <span className="text-white font-mono text-[9px] tracking-[0.4em] uppercase">Document Identity: LIT-BK-2026</span>
            </div>
