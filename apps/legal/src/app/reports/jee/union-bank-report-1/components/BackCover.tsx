'use client';

import React from 'react';

const BackgroundPattern = () => (
    <div className="absolute inset-0 pointer-events-none opacity-[0.05] z-0 overflow-hidden">
        <div className="absolute top-[-10%] right-[-10%] w-[60%] aspect-square rounded-full border-[60px] border-white" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[80%] aspect-square rounded-full border-[80px] border-[#E80000]" />
        <div className="absolute top-[20%] left-[-5%] w-[40%] aspect-square rounded-full border-[2px] border-white" />
    </div>
);

export const BackCover = () => {
    return (
        <div className="max-w-[210mm] w-full mx-auto bg-[#211B1B] shadow-2xl overflow-hidden h-[297mm] relative flex flex-col items-center justify-center shrink-0 font-sans">
            <BackgroundPattern />

            {/* Centered Logo Section */}
            <div className="relative z-10 mb-16 text-center flex flex-col items-center group">
                <div className="absolute -inset-10 bg-white/5 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-1000"></div>
                <img
                    src="/clients/jackson etti and edu logo (1).png"
                    alt="Jackson Etti & Edu"
                    className="h-32 w-auto brightness-0 invert relative z-10"
                />
            </div>

            {/* Signature JEE Red Divider */}
            <div className="w-24 h-1.5 bg-[#E80000] mb-16 relative z-10 shadow-[0_0_20px_rgba(232,0,0,0.3)]" />

            {/* Agency Info */}
            <div className="text-center space-y-8 relative z-10 px-16 max-w-2xl">
                <div className="space-y-4">
                    <h2 className="text-4xl font-serif font-black text-white uppercase">Jackson, Etti & Edu</h2>
                    <p className="text-[#E80000] font-black text-xs uppercase tracking-[0.2em] opacity-80">The Full-Service Law Firm with Sector Expertise</p>
                </div>

                <div className="pt-16 space-y-12">
                    <div className="space-y-3 text-white/70 font-medium">
                        <p className="text-lg">RCO Court, 3-5 Sinari Daranijo Street</p>
                        <p className="text-md opacity-60">Off Ajose Adeogun, Victoria Island, Lagos, Nigeria</p>
                    </div>

                    <div className="grid grid-cols-2 gap-20 pt-16 border-t border-white/5 w-full max-w-3xl">
                        <div className="flex flex-col gap-8 text-left border-l border-[#E80000] pl-8">
                            <div className="flex flex-col gap-2">
                                <a href="tel:+23414626841" className="text-lg font-black text-white hover:text-[#E80000] transition-colors tracking-tight whitespace-nowrap">
                                    +234 (1) 462 6841/3
                                </a>
                                <a href="tel:+23412806989" className="text-[10px] font-black text-white/30 hover:text-white transition-all uppercase tracking-[0.4em]">
                                    (1) 280 6989
                                </a>
                            </div>

                        </div>

                        <div className="flex flex-col gap-8 text-left border-l border-white/10 pl-8">
                            <div className="flex flex-col gap-2">
                                <a href="https://jee.africa" target="_blank" className="text-lg font-black text-white hover:text-[#E80000] transition-colors tracking-tight uppercase">
                                    jee.africa
                                </a>
                                <a href="mailto:jee@jee.africa" className="text-[10px] font-black text-white/30 hover:text-white transition-all uppercase tracking-[0.4em]">
                                    jee@jee.africa
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            {/* Footer Copyright */}
            <div className="absolute bottom-16 w-full text-center">
                <p className="text-[9px] font-black uppercase tracking-[0.6em] text-white/20">© 2026 Jackson, Etti & Edu. All Rights Reserved.</p>
            </div>
        </div>
    );
};
