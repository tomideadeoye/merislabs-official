...
function PspPresentationContent() {
    ...
    const allSlides = [
        ...
        // Section 01
        { component: <SectionHeaderSlide sectionNumber={1} title="Executive Summary" subtitle="Strategic rationale & financial target" bgLetter="E" />, hideHeader: true },
        { component: <ExecutiveSummarySlide />, hideHeader: false },
        // Section 02
        { component: <SectionHeaderSlide sectionNumber={2} title="Personal Case" subtitle="Commercial impact & contributions" bgLetter="P" />, hideHeader: true },
        { component: <PersonalCaseSlide />, hideHeader: false },
        // Section 03
        { component: <SectionHeaderSlide sectionNumber={3} title="Three Platforms" subtitle="The proposed business model" bgLetter="3" />, hideHeader: true },
        { component: <PlatformsSlide />, hideHeader: false },
        // Section 04
        { component: <SectionHeaderSlide sectionNumber={4} title="Implementation" subtitle="Five strategic workstreams · 24-month cycle" bgLetter="I" />, hideHeader: true },
        { component: <ImplementationSlide />, hideHeader: false },
        // Section 05
        { component: <SectionHeaderSlide sectionNumber={5} title="Financials" subtitle="Investment framework & ROI" bgLetter="F" />, hideHeader: true },
        { component: <FinancialSlide />, hideHeader: false },
        // Section 06
        { component: <SectionHeaderSlide sectionNumber={6} title="Success Factors" subtitle="KPIs & performance metrics" bgLetter="S" />, hideHeader: true },
        { component: <SuccessFactorsSlide />, hideHeader: false },
        // Section 07
        { component: <SectionHeaderSlide sectionNumber={7} title="Conclusion" subtitle="Commitment to firm growth" bgLetter="C" />, hideHeader: true },
    ...
    return (
        <NavigationContext.Provider value={{ goToSlideById, currentSlide, totalSlides: allSlides.length, viewMode }}>
                        ...
                        <div className="relative group w-full h-full flex items-center justify-center overflow-hidden">
                            ...
                            >
                                ...
                                <div className="deck-controls absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center space-x-8 bg-[#F9F7ED]/90 backdrop-blur-xl shadow-2xl border border-[#211B1B]/10 px-8 py-3 rounded-full z-[1000] opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                                    <button onClick={prevSlide} disabled={currentSlide === 0} className="text-[#211B1B]/40 hover:text-red-500 disabled:opacity-10 transition-colors p-2">
                                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7" /></svg>
                                    </button>
                                    <div className="flex items-center space-x-4">
                                        <div className="h-1 w-12 bg-[#211B1B]/10 rounded-full relative">
                                            <div className="absolute top-0 left-0 h-full bg-red-600 rounded-full transition-all duration-300" style={{ width: `${((currentSlide + 1) / allSlides.length) * 100}%` }} />
                                        </div>
                                        <span className="text-[#211B1B]/40 font-mono text-xs font-bold">
                                            {String(currentSlide + 1).padStart(2, '0')} / {allSlides.length}
                                        </span>
                                    </div>
                                    <button onClick={nextSlide} disabled={currentSlide === allSlides.length - 1} className="text-white bg-red-600 hover:bg-red-700 disabled:bg-[#211B1B]/5 disabled:text-[#211B1B]/20 px-6 py-2 rounded-full shadow-lg transition-all flex items-center space-x-3">
                                ...
                                {/* Side nav zones */}
                                <div onClick={prevSlide} className="absolute left-0 top-0 bottom-0 w-32 cursor-pointer z-[500] hidden sm:block group/prev">
                                ...
                                <div onClick={nextSlide} className="absolute right-0 top-0 bottom-0 w-32 cursor-pointer z-[500] hidden sm:block group/next">
                                    <div className="absolute right-8 top-1/2 -translate-y-1/2 opacity-0 group-hover/next:opacity-20 transition-opacity">
                                        <svg className="w-12 h-12 text-[#211B1B]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1" d="M9 5l7 7-7 7" /></svg>
                                    </div>
                                </div>
            ...
            <style jsx global>{`
                ...
                ::-webkit-scrollbar-thumb:hover { background: rgba(232,0,0,0.4); }
            `}</style>
