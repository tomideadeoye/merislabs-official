...
import { generateLLMResponse, AVAILABLE_ORION_TOOLS } from '@repo/shared';
...
async function executeToolCall(toolCall: any, req: NextRequest): Promise<any> {
  ...
  try {
    if (toolName === "search_orion_memory") {
      const searchResponse = await fetch(
        `${internalApiUrlBase}/api/orion/memory/search`,
        {
          ...
          body: JSON.stringify({
            ...
            filter: {
              must: [
                ...(toolArgs.memorySourceTypes?.map((t: string) => ({
                  key: "type",
                  match: { value: t },
                })) || []),
                ...(toolArgs.memorySourceTags?.map((t: string) => ({
                  key: "tags",
      ...
      return todoData.success
        ? todoData.todo
        : { error: todoData.error || "Failed to create Habitica task" };
    } else {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (let i = 0; i < MAX_ITERATIONS; i++) {
      ...
      try {
        const { requestType, messages, modelOverride, temperature, maxTokens } =
          llmParams;
