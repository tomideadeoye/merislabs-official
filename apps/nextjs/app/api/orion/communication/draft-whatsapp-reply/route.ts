...
export async function POST(request: NextRequest) {
  ...
  console.log(
    `[API /draft-whatsapp-reply] [INFO] Received request at ${new Date().toISOString()}`
  );
  const session = await auth();
  if (!session || !session.user) {
    console.error(
      `[API /draft-whatsapp-reply] [ERROR] Unauthorized access attempt.`
    );
  ...
  try {
    ...
    console.log(`[API /draft-whatsapp-reply] [INFO] Request body received`, {
    ...
    if (!messageToReplyTo || !topicOrGoal) {
      console.error(
        "[API /draft-whatsapp-reply] [ERROR] Validation failed: Missing messageToReplyTo or topicOrGoal."
      );
      return NextResponse.json(
        {
          success: false,
    ...
    console.log(
      "[API /draft-whatsapp-reply] [INFO] Consulting memory for similar contexts..."
    );
    let relevantMemories: string[] = [];
    try {
      const internalApiUrlBase =
        process.env.NEXTAUTH_URL || "http://localhost:3000";
      const memorySearchResponse = await fetch(
        `${internalApiUrlBase}/api/orion/memory/search`,
        {
          method: "POST",
          headers: {
      ...
      if (
        ...
        relevantMemories = memoryData.results.map(
          (point: ScoredMemoryPoint) => point.payload.text
        );
        console.log(
          `[API /draft-whatsapp-reply] [INFO] Found ${relevantMemories.length} relevant memories.`
        );
      } else {
        console.log(
          "[API /draft-whatsapp-reply] [INFO] No relevant memories found."
        );
      }
    } catch (memError) {
      console.warn(
        "[API /draft-whatsapp-reply] [WARN] Could not consult memory, proceeding without it.",
        memError
      );
    }
    ...
    console.log(
      "[API /draft-whatsapp-reply] [INFO] Constructing rich LLM prompt..."
    );
    ...
    const userObjectivePrompt = `
        ...
        **Message I am Replying To:**
        ...
        ${
          messageHistory
            ? `**Additional Chat History for Context:**\n"""\n${messageHistory}\n"""`
            : ""
        }
        ...
        **Relevant Past Interactions/Notes from My Memory (Use these for style and context):**
        ...
        ${
          additionalInstructions
            ? `**My Specific Instructions for This Draft:** ${additionalInstructions}`
            : ""
        }
    ...
    const messages = [
      ...
      { role: "user", content: userObjectivePrompt },
    ];
    ...
    console.log(
      `[API /draft-whatsapp-reply] [INFO] LLM call completed in ${llmDuration}ms.`
    );
    ...
    try {
      ...
      console.error(
        "[API /draft-whatsapp-reply] [ERROR] Failed to parse LLM JSON output:",
        parseErr,
        llmContent
      );
      return NextResponse.json(
    ...
    console.log(
      `[API /draft-whatsapp-reply] [INFO] Request completed successfully in ${duration}ms.`
    );
    return NextResponse.json({ success: true, drafts, relevantMemories });
  } catch (error: any) {
    const duration = Date.now() - startTime;
    console.error(
      `[API /draft-whatsapp-reply] [ERROR] An unexpected error occurred after ${duration}ms:`,
      error
    );
    return NextResponse.json(
      {
