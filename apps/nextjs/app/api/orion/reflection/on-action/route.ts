...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[ACTION_REFLECTION_API][VERBOSE] Received POST request with body:`,
    ...
    console.log(
      `[ACTION_REFLECTION_API][VERBOSE] Generating embeddings for reflectionText: "${reflectionText}"`
    );
    const embeddingResponse = await fetch(
      `${internalApiUrlBase}/api/orion/memory/generate-embeddings`,
      {
        method: "POST",
    ...
    console.log(
      `[ACTION_REFLECTION_API][VERBOSE] Embedding response:`,
    ...
    if (
      ...
      console.error(
        `[ACTION_REFLECTION_API][ERROR] Failed to generate embeddings:`,
        embeddingData.error
      );
      throw new Error(
        embeddingData.error ||
          "Failed to generate embeddings for action reflection."
      );
    }

    const embeddingVector: number[] = embeddingData.embeddings[0];
    console.log(
      `[ACTION_REFLECTION_API][VERBOSE] Embedding vector generated:`,
      embeddingVector
    );

    // 2. Prepare memory point for the Action Reflection
    const reflectionSourceId = `action_reflection_${habiticaTaskId}_${timestamp.replace(
      /[:.]/g,
      "-"
    )}`;
    console.log(
      `[ACTION_REFLECTION_API][VERBOSE] Generated reflectionSourceId: ${reflectionSourceId}`
    );
    ...
    console.log(
      `[ACTION_REFLECTION_API][VERBOSE] Memory payload:`,
    ...
    console.log(
      `[ACTION_REFLECTION_API][VERBOSE] Memory point to upsert:`,
      JSON.stringify(memoryPoint, null, 2)
    );

    // 3. Upsert the Action Reflection into memory
    const upsertResponse = await fetch(
      `${internalApiUrlBase}/api/orion/memory/upsert`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
    ...
    console.log(
      `[ACTION_REFLECTION_API][VERBOSE] Upsert response:`,
      JSON.stringify(upsertData, null, 2)
    );

    if (!upsertData.success) {
      console.error(
        `[ACTION_REFLECTION_API][ERROR] Failed to upsert memory:`,
        upsertData.error
      );
      throw new Error(
        upsertData.error || "Failed to save action reflection to memory."
      );
    }
    ...
    console.log(
      `[ACTION_REFLECTION_API][VERBOSE] Action reflection saved. Source ID: ${reflectionSourceId}`
    );
