...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const stakeholders: Stakeholder[] = await findPotentialStakeholders(
      company,
      roles
    );
...
async function findPotentialStakeholders(
  ...
  const mockStakeholders: Stakeholder[] = [
    {
      ...
      role: "HR Manager",
      company: company,
      linkedIn: `https://linkedin.com/in/hr-manager-${company
        .toLowerCase()
        .replace(/\\s+/g, "-")}`,
    },
    {
  ...
  if (roles.length > 0) {
    return mockStakeholders.filter((s) =>
      roles.some((role) => s.role.toLowerCase().includes(role.toLowerCase()))
    );
  }
...
Draft a concise and professional LinkedIn connection request OR a short introductory email to ${
    stakeholder.name
  }, ${stakeholder.role} at ${company}.
...
`;
  ...
  try {
    ...
    if (!llmResponseData.success || !llmResponseData.content) {
      throw new Error(
        llmResponseData.error || "LLM failed to generate outreach draft."
      );
    }
