...
// import { DraftApplicationRequestBody, DraftApplicationResponseBody } from '@repo/shared';
...
export async function POST(
  ...
  try {
    ...
    const numberOfDrafts = Math.max(
      1,
      Math.min(Number(reqBody.numberOfDrafts) || 3, 5)
    );
    ...
    if (!opportunityResult.success) {
      return NextResponse.json(
        { success: false, error: opportunityResult.error },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      if (memoryResponse.ok) {
        ...
        if (memoryData.success && memoryData.results) {
          ...
          console.log(
            "[DRAFT_API] Successfully fetched memory results.",
            memoryResults.length
          );
        } else {
          console.warn(
            "[DRAFT_API] Memory search proxy returned success: false or no results.",
            memoryData
          );
        }
      } else {
        ...
        if (researchResponse.ok) {
          ...
          if (
            ...
            companyWebContext = researchData.results
              ...
              .join("\n\n---\n\n");
            console.log(
              "[DRAFT_API] Successfully fetched company web context.",
              companyWebContext?.length
            );
          } else {
            console.warn(
              "[DRAFT_API] Company web search proxy returned success: false or no results.",
              researchData
            );
          }
        } else {
    ...
    for (let i = 0; i < numberOfDrafts; i++) {
      const prompt = `Draft application materials (variation ${
        i + 1
      }) for the following job OrionOpportunity, tailored to the user's profile and relevant memories.${
        companyWebContext ? " Also use the provided company web context." : ""
      ...
      console.info("[DRAFT_APPLICATION_API] Generated draft", {
        idx: i + 1,
        length: llmResponseContent?.length,
      });
    }
    ...
    return NextResponse.json(
      {
        ...
        error:
          error.message ||
          "An unexpected error occurred during application drafting.",
      },
      { status: 500 }
    );
  }
