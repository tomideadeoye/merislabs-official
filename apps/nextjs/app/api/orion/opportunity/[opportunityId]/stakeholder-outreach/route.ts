...
export async function POST(
  ...
  try {
    ...
    if (!OrionOpportunity) {
      console.error(
        "[STAKEHOLDER_OUTREACH_API] OrionOpportunity not found for ID:",
        opportunityId,
    ...
    try {
      ...
      if (memoryResponse.ok) {
        ...
        if (memoryData.success && memoryData.results) {
          ...
          console.log(
            "[OUTREACH_API] Successfully fetched memory results.",
            memoryResults.length
          );
        } else {
          console.warn(
            "[OUTREACH_API] Memory search proxy returned success: false or no results.",
            memoryData
          );
        }
      } else {
    ...
    try {
      ...
      console.log(
        "[OUTREACH_API] Successfully fetched contacts.",
        contacts.length
      );
...
`;
    ...
    console.log(
      "[STAKEHOLDER_OUTREACH_API] Sending stakeholder identification prompt to LLM..."
    );
    ...
    if (
      (identificationResponse as any).success &&
      (identificationResponse as any).content
    ) {
      identifiedStakeholders = (identificationResponse as any).content;
      console.log(
        "[STAKEHOLDER_OUTREACH_API] Identified Stakeholders:\n",
        identifiedStakeholders
      );
    } else {
      console.error(
        "[STAKEHOLDER_OUTREACH_API] LLM failed to identify stakeholders:",
        (identificationResponse as any).error
      );
...
`;
    ...
    console.log(
      "[STAKEHOLDER_OUTREACH_API] Sending outreach message drafting prompt to LLM..."
    );
    ...
    if (
      (outreachDraftingResponse as any).success &&
      (outreachDraftingResponse as any).content
    ) {
      draftOutreachMessages = (outreachDraftingResponse as any).content;
      console.log(
        "[STAKEHOLDER_OUTREACH_API] Draft Outreach Messages:\n",
        draftOutreachMessages
      );
    } else {
      console.error(
        "[STAKEHOLDER_OUTREACH_API] LLM failed to draft outreach messages:",
        (outreachDraftingResponse as any).error
      );
