...
export async function POST(request: NextRequest) {
  try {
    ...
    const { text, sourceId, tags = [], metadata = {} } = body;
    ...
    if (
      ...
      console.error(
        "[MEMORY_API] Failed to generate embeddings:",
        embeddingData.error
      );
    ...
    console.log(
      `[MEMORY_API] Preparing to upsert memory with ID: ${memoryPoint.id} and source_id: ${sourceId}`
    );
    ...
    if (!upsertData.success) {
      console.error(
        "[MEMORY_API] Failed to upsert memory to Qdrant:",
        upsertData.error
      );
    ...
    console.log(
      `[MEMORY_API] Memory successfully saved. Source ID: ${sourceId}`
    );
    ...
    if (
      ...
      try {
        ...
        console.log(
          `[MEMORY_API] Memory also saved to Neon/Postgres. ID: ${memoryPoint.id}`
        );
      } catch (pgError) {
        console.error(
          "[MEMORY_API] Neon/Postgres save error (non-critical):",
          pgError
        );
