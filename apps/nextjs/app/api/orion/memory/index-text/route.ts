...
export async function POST(req: NextRequest) {
  try {
    ...
    if (embedding.length !== 384) {
      console.warn(
        `[INDEX_TEXT_API] Embedding vector size is ${embedding.length}, expected 384.`
      );
    }
    ...
    const upsertResult = await qdrantClient.upsert(
      ORION_MEMORY_COLLECTION_NAME,
      {
        points: [
          {
    ...
    if (
      upsertResult.status !== "completed" &&
      upsertResult.status !== "acknowledged"
    ) {
    ...
    console.info(
      `[INDEX_TEXT_API] Memory point indexed successfully: ${sourceId}`
    );
