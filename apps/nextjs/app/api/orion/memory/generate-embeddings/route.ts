...
import { PYTHON_API_URL } from "@repo/shared/orion_config";
...
export async function POST(request: NextRequest) {
  ...
  if (!PYTHON_API_URL) {
    return NextResponse.json(
      { success: false, error: "Embedding service is not configured." },
      { status: 500 }
    );
  }
  ...
  if (!pythonResponse.ok) {
    const errorBody = await pythonResponse
      .json()
      .catch(() => ({ detail: pythonResponse.statusText }));
    return NextResponse.json(
      {
        success: false,
