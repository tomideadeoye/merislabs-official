...
export async function POST() {
  ...
  console.info(
    `[MEMORY_INIT][START] Received request to initialize memory system.`,
    logContext
  );

  try {
    console.log(
      "[MEMORY_INIT][INFO] Attempting to create Qdrant client instance.",
      logContext
    );

    if (!QDRANT_HOST) {
      console.error(
        "[MEMORY_INIT][ERROR] QDRANT_HOST is not defined in environment variables.",
        logContext
      );
    ...
    console.log(
      `[MEMORY_INIT][INFO] Attempting to connect to Qdrant at host: ${QDRANT_HOST}`,
      logContext
    );
    ...
    console.log(
      "[MEMORY_INIT][INFO] Qdrant client created. Checking connectivity with Qdrant cluster.",
      logContext
    );
    ...
    console.error(
      `[MEMORY_INIT][FATAL] A critical error occurred during memory system initialization.`,
