  ...
  try {
    ...
    if (tagsParam) {
      try {
        ...
        return NextResponse.json(
          { success: false, error: "Malformed tags parameter" },
          { status: 400 }
        );
      }
    }
    ...
    if (type && !BLOCK_TYPES.includes(type)) {
      return NextResponse.json(
        { success: false, error: `Invalid block type: ${type}` },
        { status: 400 }
      );
    }
    ...
    console.log(
      "[BLOCKS_LIST_API] Raw search results:",
    ...
    console.log(
      "[BLOCKS_LIST_API] Final blocks array:",
