...
function chunkText(
  text: string,
  chunkSize: number = MAX_CHUNK_SIZE_CHARS
): string[] {
  const chunks: string[] = [];
...
async function processAndIndexFile(filePath: string): Promise<{
  ...
  try {
    ...
    const embeddingResponse = await fetch(
      "/api/orion/memory/generate-embeddings",
      {
        method: "POST",
        headers: {
    ...
    if (
      ...
      embeddingData.embeddings.length !== textChunks.length
    ) {
      throw new Error(
        embeddingData.error || `Failed to generate embeddings for ${filePath}`
      );
    }

    // Create memory points
    const fileSourceIdPrefix = `localfile_${path
      .basename(filePath)
    ...
    const memoryPoints = textChunks.map((chunk, index) => {
      ...
      const source_id = `${fileSourceIdPrefix}_chunk${index}_${pointId.substring(
        0,
        4
      )}`;
      ...
      return {
        ...
        payload: {
          ...
          type: `local_doc_${
            path.extname(filePath).replace(".", "") || "generic"
          }`,
    ...
    if (!upsertData.success) {
      throw new Error(
        upsertData.error || `Failed to upsert chunks for ${filePath}`
      );
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (stats.isFile()) {
      ...
      console.log(
        `[INDEX_PATH_API] Indexing directory (top-level files only): ${pathToIndex}`
      );
