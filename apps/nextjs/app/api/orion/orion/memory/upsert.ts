...
export default async function handler(
  ...
  try {
    ...
    console.log(
      `[UPSERT_API] Upserting ${points.length} points to collection: ${collectionName}`
    );
    ...
    try {
      ...
      if (!collectionExists) {
        console.log(
          `[UPSERT_API] Collection ${collectionName} does not exist. Creating...`
        );
        ...
        console.log(
          `[UPSERT_API] Collection ${collectionName} created successfully.`
        );
      }
    } catch (error) {
    ...
    console.log(
      `[UPSERT_API] Upsert successful. Operation ID: ${upsertResult.operation_id}`
    );
