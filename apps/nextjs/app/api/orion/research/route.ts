...
import { PYTHON_API_URL } from '@shared/lib/orion_config'; // Import Python API URL
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (type === 'scrape') {
      if (!url) {
        logger.warn('[RESEARCH_PROXY] URL is required for scrape type.', { type, url });
      ...
      requestBody = { url };
      logger.info('[RESEARCH_PROXY] Proxying scraping request to Python backend', { pythonBackendUrl, url });
    } else {
      if (!query) {
        logger.warn('[RESEARCH_PROXY] Search query is required for web/local search.', { type, query });
      ...
      logger.info('[RESEARCH_PROXY] Proxying search request to Python backend', { pythonBackendUrl, query, count });
    }
    ...
    try {
      ...
      logger.error('[RESEARCH_PROXY_ERROR] Failed to fetch from Python backend', { pythonBackendUrl, error: errorMsg, raw: fetchError });
      return NextResponse.json({ success: false, error: errorMsg }, { status: 502 });
    }

    if (!pythonResponse.ok) {
      logger.error('[RESEARCH_PROXY] Python backend failed', { status: pythonResponse.status, statusText: pythonResponse.statusText });
      try {
    ...
    logger.success('[RESEARCH_PROXY] Python backend response received', { pythonBackendUrl });
    return NextResponse.json({ success: true, results: pythonData });

  } catch (error: any) {
    logger.error('[RESEARCH_PROXY_ERROR] Unexpected error in research proxy', { error: error?.message, raw: error });
