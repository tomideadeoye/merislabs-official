...
import { generateLLMResponse } from '@shared/lib/orion_llm'; // Import LLM utility
import { fetchCVComponents } from '@shared/lib/cv'; // Import function to fetch all CV components
import { getCVComponentsFromNotion } from '@shared/lib/notion_service'; // Import from notion_service
import { CV_COMPONENT_SELECTION_REQUEST_TYPE } from '@shared/lib/orion_config'; // Import request type
import { CVComponentShared } from '@shared/types/orion'; // Import the shared CV component type
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`[${new Date().toISOString()}] [INFO] [CV Suggest] POST /api/orion/cv/suggest-components - Params:`, { job_title, company_name, jd_analysis: jd_analysis?.slice(0, 100) + '...' });

    if (!jd_analysis) {
      console.warn(`[${new Date().toISOString()}] [WARN] [CV Suggest] Missing JD analysis in request body.`);
    ...
    console.log(`[${new Date().toISOString()}] [INFO] [CV Suggest] Fetching CV components from Notion...`);
    const allComponents = await getCVComponentsFromNotion();

    if (!allComponents || allComponents.length === 0) {
      console.warn(`[${new Date().toISOString()}] [WARN] [CV Suggest] No CV components available for suggestion.`);
    ...
    console.log(`[${new Date().toISOString()}] [INFO] [CV Suggest] Sending suggestion prompt to LLM...`);
    ...
    if (llmResponse && typeof llmResponse === 'string') {
      ...
      console.log(`[${new Date().toISOString()}] [INFO] [CV Suggest] Suggested component IDs:`, validSuggestedIds);
    ...
    console.error(`[${new Date().toISOString()}] [ERROR] [CV Suggest] Exception:`, error);
    const duration = Date.now() - startTime;
    return NextResponse.json({
      success: false,
