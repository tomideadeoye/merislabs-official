...
import {
  NOTION_API_KEY,
  NOTION_DATABASE_ID,
} from "@repo/shared/orion_server_config";
import type { CVComponentShared } from "@repo/shared";
  ...
  try {
    ...
    if (!components || !Array.isArray(components) || components.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: "Invalid or empty components array in request body.",
        },
        { status: 400 }
      );
    }
      ...
      try {
        ...
        if (componentData.component_name)
          properties["Component Name"] = {
            title: [{ text: { content: componentData.component_name } }],
          };
        if (componentData.component_type)
          properties["Component Type"] = {
            rich_text: [{ text: { content: componentData.component_type } }],
          };
        if (componentData.content_primary)
          properties["Content (Primary)"] = {
            rich_text: [{ text: { content: componentData.content_primary } }],
          };
        if (componentData.associated_company_institution)
          properties["Associated Company/Institution"] = {
            rich_text: [
              {
                text: { content: componentData.associated_company_institution },
              },
            ],
          };
        if (componentData.start_date)
          properties["Start Date"] = {
            date: { start: componentData.start_date },
          };
        if (componentData.end_date)
          properties["End Date"] = { date: { start: componentData.end_date } };
        if (componentData.keywords)
          properties["Keywords"] = {
        ...
        if (!properties["Component Name"] || !properties["Content (Primary)"]) {
          ...
          console.warn(
            `Skipping CV component due to missing required fields: ${
              componentData.component_name || "Untitled"
            }`
          );
          errors.push({
            component: componentData.component_name || "Untitled",
            error: "Missing required fields (Component Name or Content).",
          }); // Use Notion property names in error
        ...
        console.error(
          `Error creating Notion page for CV component: ${
            componentData.component_name || "Untitled"
          }`,
