...
export async function GET() {
  try {
    ...
    logger.error('Error fetching opportunities', { error: errorMessage });
    return NextResponse.json({ error: 'Failed to fetch opportunities', details: errorMessage }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
      ...
      await Promise.all(
        validatedData.stakeholders.map((stakeholder) =>
          query(
            ...
            [newOpportunity.id, stakeholder.name, stakeholder.title, stakeholder.email, stakeholder.linkedin]
          )
        )
      );
    }
    ...
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
    logger.error('Error creating OrionOpportunity', { error: errorMessage });
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: 'Invalid OrionOpportunity data', details: error.errors }, { status: 400 });
    }
    return NextResponse.json({ error: 'Failed to create OrionOpportunity', details: errorMessage }, { status: 500 });
  }
}
